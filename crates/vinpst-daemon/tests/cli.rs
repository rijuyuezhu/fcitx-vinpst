...
impl TempConfig {
    fn write(name: &str, contents: &str) -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "vinpst-daemon-test-{}-{unique}-{name}.json",
            std::process::id()
        ));
...
impl TempBytes {
    fn write(name: &str, extension: &str, contents: &[u8]) -> Self {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let path = std::env::temp_dir().join(format!(
            "vinpst-daemon-test-{}-{unique}-{name}.{extension}",
            std::process::id()
        ));
...
fn daemon_command() -> Command {
    let mut command = Command::new(env!("CARGO_BIN_EXE_vinpst-daemon"));
    command.env(
        "XDG_CONFIG_HOME",
        std::env::temp_dir().join(format!(
            "vinpst-daemon-test-empty-config-{}",
            std::process::id()
        )),
    );
    command
...
fn print_config_accepts_committed_default_fixture() {
    let value = assert_json_success(
        run_daemon_with_config(
            default_config_path(),
            &["print-config"],
            "run vinpst-daemon print-config on default fixture",
        ),
        "print-config output",
    );
...
fn print_config_discovers_xdg_user_config_without_explicit_path() {
    ...
    let config_dir = config_home.path().join("fcitx-vinpst");
    ...
    let output = daemon_command()
        ...
        .arg("print-config")
        .output()
        .expect("run vinpst-daemon with discovered user config");
...
fn explicit_config_overrides_discovered_xdg_user_config() {
    ...
    let output = daemon_command()
        ...
        .arg("print-config")
        .output()
        .expect("run vinpst-daemon with explicit config override");
...
fn print_config_with_default_fixture_ignores_configured_backend_runtime_init() {
    let value = assert_json_success(
        run_daemon_with_config(
            ...
            "run vinpst-daemon print-config with configured default fixture",
        ),
        "config",
    );
...
fn print_config_summary_omits_sensitive_config_details() {
    ...
    let stdout = assert_success_stdout(
        run_daemon_with_config(
            &config.path,
            &["--configured-backends", "print-config"],
            "run vinpst-daemon print-config",
        ),
        "print-config output",
    );
...
fn runtime_status_reports_initialized_mock_runtime() {
    let value = assert_json_success(
        run_daemon_with_config(
            default_config_path(),
            &["runtime-status"],
            "run vinpst-daemon runtime-status on default fixture",
        ),
        "runtime status",
    );
    assert_eq!(value["ok"], true);
    ...
    assert_eq!(value["dbus"]["service"], "org.fcitx.Vinpst");
    assert_eq!(value["dbus"]["object_path"], "/org/fcitx/Vinpst");
    assert_eq!(value["dbus"]["interface"], "org.fcitx.Vinpst.Service");
...
fn runtime_status_reports_configured_command_demo_runtime() {
    let value = assert_json_success(
        run_daemon_with_config(
            ...
            "run vinpst-daemon runtime-status with configured command demo",
        ),
...
fn audio_devices_reports_default_capture_target_and_unavailable_backend() {
    let value = assert_json_success(
        run_daemon_with_config(
            default_config_path(),
            &["audio-devices"],
            "run vinpst-daemon audio-devices on default fixture",
        ),
        "audio devices",
    );
    assert_eq!(value["ok"], true);
...
fn audio_devices_preserves_configured_capture_target_object() {
    ...
    let value = assert_json_success(
        run_daemon_with_config(
            &config.path,
            &["audio-devices"],
            "run vinpst-daemon audio-devices with object capture target",
        ),
        "audio devices",
    );
    assert_eq!(value["ok"], true);
...
fn audio_devices_reports_pipewire_enumeration_error_without_failing() {
    let config_dir = std::env::temp_dir().join(format!(
        "vinpst-daemon-missing-pipewire-config-{}-{}",
        std::process::id(),
    ...
    let output = daemon_command()
        ...
        .arg("audio-devices")
        .output()
        .expect("run vinpst-daemon audio-devices without PipeWire client config");
...
fn once_pipewire_backend_reports_recorder_setup_error_with_plan() {
    ...
    let output = daemon_command()
        ...
        .output()
        .expect("run vinpst-daemon once with pipewire recorder");
    fs::remove_dir(&config_dir).expect("remove empty PipeWire config dir");
...
fn asr_state_accepts_committed_default_fixture() {
    let value = assert_json_success(
        run_daemon_with_config(
            default_config_path(),
            &["asr-state"],
            "run vinpst-daemon asr-state on default fixture",
        ),
        "ASR state",
    );
...
fn asr_state_with_default_fixture_ignores_configured_backend_runtime_init() {
    let value = assert_json_success(
        run_daemon_with_config(
            ...
            "run vinpst-daemon asr-state with configured default fixture",
        ),
        "ASR state",
    );
    assert_eq!(value["target_provider_id"], "sherpa-onnx");
...
fn asr_state_uses_config_file() {
    ...
    let value = assert_json_success(
        run_daemon_with_config(&config.path, &["asr-state"], "run vinpst-daemon asr-state"),
        "ASR state",
    );
...
fn print_config_ignores_configured_backend_runtime_init() {
    let value = assert_json_success(
        run_daemon(
            &["--configured-backends", "print-config"],
            "run vinpst-daemon --configured-backends print-config",
        ),
        "config",
    );
    assert_eq!(value["version"], 1);
...
fn asr_state_ignores_configured_backend_runtime_init() {
    let value = assert_json_success(
        run_daemon(
            &["--configured-backends", "asr-state"],
            "run vinpst-daemon --configured-backends asr-state",
        ),
        "ASR state",
    );
    assert_eq!(value["target_provider_id"], "sherpa-onnx");
...
fn text_adapters_accepts_committed_default_fixture() {
    let value = assert_json_success(
        run_daemon_with_config(
            default_config_path(),
            &["text-adapters"],
            "run vinpst-daemon text-adapters on default fixture",
        ),
        "text adapter state",
    );
...
fn text_adapters_with_default_fixture_ignores_configured_backend_runtime_init() {
    let value = assert_json_success(
        run_daemon_with_config(
            ...
            "run vinpst-daemon text-adapters with configured default fixture",
        ),
...
fn text_adapters_uses_config_file() {
    ...
    let value = assert_json_success(
        run_daemon_with_config(
            &config.path,
            &["text-adapters"],
            "run vinpst-daemon text-adapters",
        ),
        "text adapter diagnostics",
    );
...
fn text_adapters_ignores_configured_backend_runtime_init() {
    let value = assert_json_success(
        run_daemon(
            &["--configured-backends", "text-adapters"],
            "run vinpst-daemon --configured-backends text-adapters",
        ),
        "text adapter diagnostics",
    );
    assert_eq!(value["adapter_count"], 0);
...
fn once_can_use_configured_backends() {
    ...
    let value = assert_json_success(
        run_daemon_with_config(
            &config.path,
            &["--configured-backends", "--once"],
            "run vinpst-daemon --once with configured backends",
        ),
        "recognition payload",
    );
...
fn once_can_use_configured_openai_provider_over_http() {
    ...
    let value = assert_json_success(
        run_daemon_with_config(
            &config.path,
            &["--configured-backends", "--once"],
            "run vinpst-daemon --once with configured OpenAI-compatible provider",
        ),
        "recognition payload",
    );
...
fn once_can_read_pcm_file_into_configured_command_pipeline() {
    ...
    let value = assert_json_success(
        run_daemon_with_config(
            ...
            "run vinpst-daemon --once with PCM and configured command pipeline",
        ),
        "recognition payload",
    );
...
fn once_can_read_wav_file_into_configured_command_asr() {
    ...
    let value = assert_json_success(
        run_daemon_with_config(
            ...
            "run vinpst-daemon --once with WAV and configured command ASR",
        ),
        "recognition payload",
    );
...
fn dbus_allows_wav_file_argument_for_service_runtime() {
    ...
    let stderr = assert_failure_stderr(
        run_daemon(
            ...
            "run vinpst-daemon print-config with bare WAV file",
        ),
    ...
    let value = assert_json_success(
        run_daemon_with_config(
            ...
            "run vinpst-daemon print-config with D-Bus WAV file",
        ),
...
fn record_ms_requires_once_mode() {
    let stderr = assert_failure_stderr(
        run_daemon(
            ...
            "run vinpst-daemon print-config with record delay",
        ),
...
fn once_accepts_record_ms_delay() {
    let value = assert_json_success(
        run_daemon(
            &["--once", "--record-ms", "1"],
            "run vinpst-daemon --once with record delay",
        ),
        "recognition payload",
    );
...
fn once_rejects_odd_pcm_file() {
    ...
    let output = run_daemon(
        ...
        "run vinpst-daemon --once with odd PCM file",
    );
    let stderr = assert_failure_stderr(output, "vinpst-daemon --once with odd PCM file");
...
fn once_reports_ambiguous_configured_text_adapters() {
    ...
    let output = run_daemon_with_config(
        &config.path,
        &["--configured-backends", "--once"],
        "run vinpst-daemon --once with ambiguous configured backends",
    );

    let stderr = assert_failure_stderr(
        output,
        "vinpst-daemon --once with ambiguous configured backends",
    );
...
fn once_reports_missing_configured_text_adapter() {
    ...
    let output = run_daemon_with_config(
        &config.path,
        &["--configured-backends", "--once"],
        "run vinpst-daemon --once with missing configured text adapter",
    );
...
fn text_adapters_reports_empty_adapter_summary() {
    ...
    let value = assert_json_success(
        run_daemon_with_config(
            &config.path,
            &["text-adapters"],
            "run vinpst-daemon text-adapters without adapters",
        ),
...
fn text_adapters_reports_multiple_adapter_ids() {
    ...
    let value = assert_json_success(
        run_daemon_with_config(
            &config.path,
            &["text-adapters"],
            "run vinpst-daemon text-adapters with multiple adapters",
        ),
        "text adapter summary",
    );
