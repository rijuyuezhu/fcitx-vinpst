...
pub const REMOTE_TEXT_PROVIDER_ID: &str = "provider.vinpst.remote.streaming";
...
pub fn remote_text_settings(
    ...
    let explicit_port = trimmed_env(provider, "VINPST_ASR_PORT");
    let port = if let Some(port) = explicit_port {
        parse_port(&port)?
    } else {
        trimmed_env(provider, "VINPST_ASR_URL")
            .as_deref()
            .and_then(port_from_websocket_url)
            .unwrap_or(DEFAULT_PORT)
    };
    let debounce_ms = trimmed_env(provider, "VINPST_ASR_DEBOUNCE_MS")
        .map_or(Ok(DEFAULT_DEBOUNCE_MS), |value| parse_debounce(&value))?;
    let api_key = trimmed_env(provider, "VINPST_ASR_API_KEY")
...
fn trimmed_env(provider: &vinpst_config::AsrProviderConfig, name: &str) -> Option<String> {
...
pub enum RemoteTextSettingsError {
    ...
    #[error("VINPST_ASR_PORT must be between 1 and 65535, got `{0}`")]
    InvalidPort(String),
    /// Debounce is not a positive integer.
    #[error("VINPST_ASR_DEBOUNCE_MS must be a positive integer, got `{0}`")]
    InvalidDebounce(String),
    /// Remote input must be protected by an API key.
    #[error("remote ASR provider requires VINPST_ASR_API_KEY")]
    MissingApiKey,
}
...
mod tests {
    ...
    fn config_with_remote_env(env: &serde_json::Value) -> VinpstConfig {
    ...
    fn settings_are_disabled_for_other_active_providers() {
        let config = VinpstConfig::bundled_default().expect("parse bundled config");
        assert_eq!(remote_text_settings(&config).unwrap(), None);
    }

    #[test]
    fn settings_apply_legacy_defaults_and_redact_debug() {
        let config = config_with_remote_env(&json!({"VINPST_ASR_API_KEY":"fixture-key"}));
    ...
    fn settings_accept_explicit_and_url_derived_ports() {
        let explicit = config_with_remote_env(&json!({
            ...
            "VINPST_ASR_PORT":"9001",
            "VINPST_ASR_URL":"ws://127.0.0.1:9002/v1/realtime",
            "VINPST_ASR_DEBOUNCE_MS":"250"
        }));
        ...
        let derived = config_with_remote_env(&json!({
            "VINPST_ASR_API_KEY":"key",
            "VINPST_ASR_URL":"ws://[::1]:9010/v1/realtime"
        }));
    ...
    fn realtime_authorization_is_loopback_and_bearer_protected() {
        let config = config_with_remote_env(&json!({"VINPST_ASR_API_KEY":"key"}));
        let settings = remote_text_settings(&config).unwrap().unwrap();
        assert_eq!(
