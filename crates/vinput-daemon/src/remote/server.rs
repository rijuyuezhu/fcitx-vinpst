...
<body>
  ...
  <textarea id="editor" disabled placeholder="Connect VInput, then type here."></textarea>
  ...
  <script>
    ...
    function apiKey() {
      ...
      if (fromHash) {
        localStorage.setItem('vinput_remote_api_key', fromHash)
        history.replaceState(null, '', location.pathname)
        return fromHash
      }
      return localStorage.getItem('vinput_remote_api_key') || prompt('API key') || ''
    }
