...
const NOTIFICATION_URL_ENV: &str = "VINPUT_NOTIFICATION_URL";
...
impl NotificationTextSource for HttpNotificationTextSource {
    fn fetch(&self, url: &str) -> Result<String, ()> {
        vinput_http::fetch_json_text(url, NOTIFICATION_TIMEOUT).map_err(|_| ())
    }
}
...
fn default_read_state_path() -> Option<PathBuf> {
    ...
    Some(cache_home.join("fcitx-vinput").join(READ_STATE_FILE))
}
...
mod tests {
    ...
    fn feed(id: u64) -> String {
        serde_json::json!({
            ...
            "url": "https://example.invalid/details?from=vinput"
        })
        .to_string()
    }
    ...
    fn newer_notification_is_ready_until_acknowledged() {
        ...
        let path = directory
            .path()
            .join("cache/fcitx-vinput/read_notifications");
