...
use vinpst_config::{AsrProviderConfig, AsrProviderKind, redact_url_for_diagnostics};
use vinpst_http::{
...
const API_KEY_ENV: &str = "VINPST_ASR_API_KEY";
const MODEL_ENV: &str = "VINPST_ASR_MODEL";
const LANGUAGE_ENV: &str = "VINPST_ASR_LANGUAGE";
const PROMPT_ENV: &str = "VINPST_ASR_PROMPT";
...
mod tests {
    ...
    fn provider(endpoint: &str) -> AsrProviderConfig {
        AsrProviderConfig {
            ...
            args: Vec::new(),
            env: HashMap::from([
                ("VINPST_ASR_API_KEY".to_owned(), "secret-token".to_owned()),
                ("VINPST_ASR_LANGUAGE".to_owned(), "en".to_owned()),
                ("VINPST_ASR_PROMPT".to_owned(), "names".to_owned()),
            ]),
    ...
    fn remote_spec_uses_typed_and_legacy_environment_fields() {
        ...
        env_model
            .env
            .insert("VINPST_ASR_MODEL".to_owned(), "env-model".to_owned());
        assert_eq!(
    ...
    fn remote_spec_requires_http_endpoint_and_model() {
        ...
        missing_model.env.remove("VINPST_ASR_MODEL");
        assert!(
