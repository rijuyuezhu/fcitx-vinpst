...
pub(crate) fn resolve_online_runtime_plan(
    ...
    let metadata_path = paths.model_dir.join("vinpst-model.json");
    if !metadata_path.is_file() {
...
fn warmup_online_recognizer(
    ...
    eprintln!(
        "vinpst: sherpa-onnx online recognizer warmup completed duration_ms={ONLINE_WARMUP_DURATION_MS}"
    );
    Ok(())
}
...
struct SherpaOnnxOnlineRecognitionSession {
    ...
    pcm: Option<vinpst_audio::PcmSpec>,
...
impl crate::RecognitionSession for SherpaOnnxOnlineRecognitionSession {
    fn push_pcm(&mut self, pcm: &vinpst_audio::PcmBuffer) -> Result<(), crate::AsrError> {
        self.ensure_active()?;
    ...
    fn push_audio(&mut self, samples: &[i16]) -> Result<(), crate::AsrError> {
        self.push_pcm(&vinpst_audio::PcmBuffer::at_default_rate(samples.to_vec()))
    }
