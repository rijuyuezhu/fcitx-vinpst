...
fn backend_factory_uses_legacy_streaming_protocol_for_streaming_command_provider() {
    let script_path = write_temp_script(
        "vinpst-factory-legacy-streaming-asr",
        r"
import json
import sys
...
#[test]
fn sherpa_onnx_offline_runtime_plan_uses_vinpst_model_metadata() {
    let temp_dir = tempfile::tempdir().unwrap();
    let root = temp_dir.path();
...
",
    ...
    assert_eq!(
        plan.metadata_path,
        Some(model_dir.join("vinpst-model.json"))
    );
    assert_eq!(
        plan.layout,
        SherpaOnnxOfflineModelLayout::SenseVoice {
...
#[test]
fn sherpa_onnx_offline_runtime_plan_rejects_invalid_vinpst_model_metadata() {
    let temp_dir = tempfile::tempdir().unwrap();
    let root = temp_dir.path();
    let model_dir = root.join("bad-meta");
    std::fs::create_dir_all(&model_dir).unwrap();
    std::fs::write(model_dir.join("vinpst-model.json"), b"not json").unwrap();
    let provider = AsrProviderConfig {
        id: "sherpa-onnx".to_owned(),
        kind: AsrProviderKind::Local,
        timeout_ms: None,
    ...
    assert!(matches!(
        ...
        SherpaOnnxModelPathError::InvalidModelMetadata { path, message }
            if path.ends_with("vinpst-model.json") && message.contains("expected")
    ));
}
