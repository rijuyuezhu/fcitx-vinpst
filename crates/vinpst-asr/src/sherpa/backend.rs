...
use vinpst_config::{AsrProviderConfig, VadConfig};
...
impl SherpaOnnxBackend {
    ...
    pub fn with_config_and_vad(
        ...
        let model_root = std::env::var_os("VINPST_SHERPA_MODEL_ROOT")
        ...
        let (runtime, description, capabilities) = if online {
            ...
            let vad = vad_config.and_then(|config| {
                ...
                let Some(plan) = crate::sherpa_vad::SherpaOnnxVadPlan::resolve(config) else {
                    eprintln!(
                        "vinpst: VAD enabled but silero_vad.onnx was not found; using untrimmed audio"
                    );
                    return None;
                };
                let Some(trimmer) = crate::sherpa_vad::SherpaOnnxVadTrimmer::create(&plan) else {
                    eprintln!(
                        "vinpst: failed to load Silero VAD model {}; using untrimmed audio",
                        plan.model.display()
                    );
                    return None;
                };
    ...
    fn create_session(
        ...
        match &self.runtime {
            SherpaOnnxRuntime::Offline(runtime) => Ok(Box::new(SherpaOnnxRecognitionSession {
                ...
                pcm: vinpst_audio::PcmSpec::default(),
                samples: Vec::new(),
                events: Vec::new(),
                finished: false,
                cancelled: false,
...
impl RecognitionSession for SherpaOnnxRecognitionSession {
    fn push_pcm(&mut self, pcm: &vinpst_audio::PcmBuffer) -> Result<(), AsrError> {
        if self.cancelled {
