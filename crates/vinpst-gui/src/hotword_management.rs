...
impl HotwordEditorState {
    ...
    pub(super) fn from_config(config: &VinpstConfig, preferred_provider: Option<&str>) -> Self {
    ...
    fn select_provider(&mut self, config: &VinpstConfig, provider_id: &str) {
...
impl App {
    ...
    fn begin_hotword_path_mutation(
        ...
        crate::blocking_task::perform(
            "vinpst-gui-hotword-path-mutation",
            move || {
    ...
    fn begin_hotword_content_load(&mut self) -> Task<Message> {
        ...
        crate::blocking_task::perform(
            "vinpst-gui-hotword-content-load",
            move || {
    ...
    fn begin_hotword_content_save(&mut self) -> Task<Message> {
        ...
        crate::blocking_task::perform(
            "vinpst-gui-hotword-content-save",
            move || {
    ...
    fn begin_hotword_activation_retry(&mut self) -> Task<Message> {
        ...
        crate::blocking_task::perform(
            "vinpst-gui-hotword-activation-retry",
...
fn hotword_provider_options(config: &VinpstConfig) -> Vec<HotwordProviderSelection> {
    config
        .asr
        .providers
        .iter()
...
fn configured_hotword_path(config: &VinpstConfig, provider_id: &str) -> Option<PathBuf> {
    config
        .asr
        .providers
        .iter()
