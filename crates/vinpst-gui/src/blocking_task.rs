...
mod tests {
    ...
    async fn worker_does_not_inherit_the_tokio_runtime_context() {
        let outside_runtime = run("vinpst-gui-runtime-boundary-test", || {
    ...
    async fn blocking_http_client_runs_without_nesting_the_parent_runtime() {
        ...
        let body = run("vinpst-gui-blocking-http-test", move || {
            vinpst_http::fetch_json_text(&url, Duration::from_secs(2))
        })
        .await
        .expect("plain worker result")
