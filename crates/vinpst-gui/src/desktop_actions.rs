...
use vinpst_daemon_control::{FLATPAK_INFO_PATH_ENV, FLATPAK_SPAWN_ENV};

use crate::{App, GuiLocale, GuiText, Message, OperationState};

const DESKTOP_OPENER_ENV: &str = "VINPST_DESKTOP_OPENER";
...
fn spawn_desktop_open_target(
    ...
    if thread::Builder::new()
        .name("vinpst-desktop-opener-reaper".to_owned())
        .spawn(move || {
...
mod tests {
    ...
    fn opener_spawn_reports_only_fixed_categories() {
        let environment = DesktopOpenEnvironment::new(
            "/definitely/missing/vinpst-opener",
            "/missing",
            "flatpak-spawn",
        );
