...
impl LiveModelEntry {
    ...
    /// Returns the model family from typed `vinpst_model` metadata.
    #[must_use]
    pub fn model_family(&self) -> Option<&str> {
        self.vinpst_model
            .as_ref()
            .and_then(LiveVinpstModelMetadata::model_family)
    }
    ...
    pub fn classified_model_family(&self) -> Option<LiveModelFamily> {
        self.vinpst_model
            .as_ref()
            .and_then(LiveVinpstModelMetadata::classified_model_family)
    }

    /// Returns the backend declared by typed `vinpst_model` metadata.
    #[must_use]
...
/// Optional display metadata persisted into installed `vinpst-model.json` files.
...
/// Typed subset of live `vinpst_model` metadata while preserving unknown fields.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
pub struct LiveVinpstModelMetadata {
...
impl LiveVinpstModelMetadata {
    ...
    /// Serializes the metadata back to JSON for `vinpst-model.json` materialization.
