...
fn parses_live_models_json_sensevoice_metadata() {
    ...
    let metadata = model.vinpst_model.as_ref().unwrap();
...
fn install_plan_adds_targets_and_checksum_policy() {
    ...
    let plan = index.install_plan(&config, "/var/lib/vinpst/assets/");

    assert_eq!(plan.target_root, "/var/lib/vinpst/assets");
    ...
    assert_eq!(
        ...
        "/var/lib/vinpst/assets/models/sherpa-zh-small.tar.zst"
    );
...
fn archive_staging_paths_plan_asset_archive_tree_and_target_paths() {
    let asset = PlannedInstallAsset {
        ...
        target_path: "/var/lib/vinpst/assets/models/m.tar.zst".to_owned(),
    ...
    assert_eq!(
        ...
        std::path::PathBuf::from("/var/lib/vinpst/assets/models/m.tar.zst")
    );
}
...
fn archive_staging_paths_plan_tar_bz2_archive_tree_name() {
    let asset = PlannedInstallAsset {
        ...
        target_path: "/var/lib/vinpst/assets/models/SenseVoice.TAR.BZ2".to_owned(),
...
fn archive_staging_paths_batch_preserves_plan_order() {
    ...
    let plan = InstallPlan::from_assets(&assets, "/var/lib/vinpst/assets");
    let temp_dir = tempfile::tempdir().unwrap();
...
fn materialize_staged_tree_copies_then_renames_across_filesystems() {
    ...
    let Ok(source_root) = tempfile::Builder::new()
        .prefix("vinpst-registry-cross-device-")
        .tempdir_in("/dev/shm")
    else {
        return;
    };
...
fn install_live_model_downloads_verifies_extracts_and_materializes_single_root() {
    ...
    let metadata: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(temp_dir.path().join("models/test-sense/vinpst-model.json"))
            .unwrap(),
    )
    .unwrap();
