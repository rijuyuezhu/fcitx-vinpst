...
fn daemon_user_service_command(
    ...
    match action {
        ...
        "disable-now" => Ok(UserServiceCommand {
            program: std::env::var("VINPUT_DAEMON_SYSTEMCTL")
                .unwrap_or_else(|_| "systemctl".to_owned()),
...
fn print_daemon_prepare_remove(
    dry_run: bool,
    preflight: bool,
    json_output: bool,
) -> anyhow::Result<()> {
    ...
    let output = if dry_run {
        serde_json::json!({
            ...
            "direct_owner_guard": {
                ...
                "requires_vinput_daemon_identity": true,
            },
            "verification": {
            ...
            "next_steps": [
                ...
                "run vinput daemon prepare-remove --preflight in each live user session",
                "run vinput daemon prepare-remove only after every preflight succeeds"
            ],
        })
    } else {
...
fn removal_reload_failure(
    ...
    serde_json::json!({
        ...
        "next_steps": [
            ...
            "retry vinput daemon prepare-remove before continuing package removal",
        ],
    })
}
fn removal_session_guard(snapshot: &serde_json::Value) -> serde_json::Value {
...
fn removal_next_steps(ok: bool, systemd: bool, preflight: bool) -> Vec<&'static str> {
    match (ok, systemd, preflight) {
        (true, _, true) => vec![
            ...
            "run vinput daemon prepare-remove after all preflights succeed",
        ],
        (false, _, true) => vec![
        ...
        (false, true, false) => vec![
            ...
            "run systemctl --user status vinput-daemon.service",
        ],
