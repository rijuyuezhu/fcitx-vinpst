...
const NATIVE_SERVICE_TEMPLATE: &str = "/usr/lib/systemd/user/vinput-daemon.service";
...
pub(super) fn print_daemon_install_service(
    ...
    let value = serde_json::json!({
        ...
        "next_steps": [
            "run vinput daemon restart to start the installed service",
            "run vinput doctor to inspect Flatpak permissions and activation state"
        ],
    });
    if json_output {
