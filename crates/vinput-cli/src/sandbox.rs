...
const DEFAULT_FLATPAK_ADDON_ROOT: &str = "/app/addons/Vinput";
...
pub(crate) const FLATPAK_APP_ID_ENV: &str = "VINPUT_FLATPAK_APP_ID";
pub(crate) const FLATPAK_ADDON_ROOT_ENV: &str = "VINPUT_FLATPAK_ADDON_ROOT";
...
pub(crate) fn default_service_template_path() -> PathBuf {
    flatpak_addon_root().join("share/systemd/user/vinput-daemon.service")
}

pub(crate) fn render_flatpak_service(template: &str) -> anyhow::Result<String> {
    let app_id = flatpak_app_id();
    let daemon = flatpak_addon_root().join("bin/vinput-daemon");
...
fn render_flatpak_service_for(
    ...
    for line in template.lines() {
        if let Some(command) = line.strip_prefix("ExecStart=") {
            ...
            output.push("ExecStop=pkill -INT vinput-daemon".to_owned());
            found_stop = true;
        } else {
    ...
    if !found_stop {
        let position = output
            ...
            + 1;
        output.insert(position, "ExecStop=pkill -INT vinput-daemon".to_owned());
    }
    Ok(format!("{}\n", output.join("\n")))
}
...
mod tests {
    ...
    fn service_rewrite_preserves_args_and_adds_flatpak_stop() {
        ...
        let rendered = render_flatpak_service_for(
            template,
            "org.fcitx.Fcitx5",
            "/app/addons/Vinput/bin/vinput-daemon",
        )
