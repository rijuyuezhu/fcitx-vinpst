//! Integration tests for first-run `vinput init` behavior.
...
fn init_dry_run_json_uses_xdg_defaults_without_writes() {
    let root = unique_temp_dir("vinput-cli-init-dry-run");
    ...
    let output = vinput_command()
        ...
        .output()
        .expect("run vinput init dry-run json");
    ...
    assert_eq!(
        ...
        data_home
            .join("dbus-1/services/org.fcitx.Vinput.service")
            .to_string_lossy()
            .as_ref()
    );
    assert!(
...
fn init_writes_default_config_and_managed_dirs_idempotently() {
    let root = unique_temp_dir("vinput-cli-init-write");
    let config_home = root.join("config-home");
    let data_home = root.join("data-home");
    let cache_home = root.join("cache-home");
    let config_path = config_home.join("fcitx-vinput/config.json");
    let model_root = data_home.join("fcitx-vinput/models");
    let cache_root = cache_home.join("fcitx-vinput");
...
fn init_force_overwrites_existing_config() {
    let root = unique_temp_dir("vinput-cli-init-force");
    ...
    let output = vinput_command()
        ...
        .env("HOME", root.join("home"))
        .output()
        .expect("run vinput init without force");
...
fn init_text_output_includes_next_activation_hint() {
    let root = unique_temp_dir("vinput-cli-init-text");
    let output = vinput_command()
        .args(["init", "--dry-run"])
        .arg("--config")
        ...
        .env("HOME", root.join("home"))
        .output()
        .expect("run vinput init text dry-run");

    let stdout = assert_stdout_success(output, "init text output");
    assert!(stdout.contains("dry_run: true"));
    assert!(stdout.contains("config_will_write: true"));
    assert!(stdout.contains("activation_service_command: vinput activation-service"));
    assert!(stdout.contains("--configured-backends"));
    assert!(stdout.contains("next: vinput model install <id-or-short-id>"));
}
