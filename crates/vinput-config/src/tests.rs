...
fn output_ducking_defaults_and_normalization_match_legacy() {
    ...
    let low = VinputConfig::from_json_str(r#"{"version":1,"global":{"duck_output_volume":-0.5}}"#)
        .unwrap();
    assert!(low.global.duck_output_volume.abs() < f32::EPSILON);

    let high = VinputConfig::from_json_str(r#"{"version":1,"global":{"duck_output_volume":1.5}}"#)
        .unwrap();
