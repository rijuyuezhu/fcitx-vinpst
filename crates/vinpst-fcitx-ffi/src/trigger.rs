...
use vinpst_fcitx_core::{TriggerEvent, TriggerKind, TriggerMode, TriggerModeState};

/// Opaque trigger controller owned by Rust.
pub struct VinpstFcitxTriggerState {
...
pub struct VinpstFcitxTriggerEventView {
    /// Stable `VINPST_FCITX_TRIGGER_EVENT_*` value.
    pub kind: u8,
...
fn trigger_event(event: VinpstFcitxTriggerEventView) -> Option<TriggerEvent> {
    match event.kind {
...
#[unsafe(no_mangle)]
pub extern "C" fn vinpst_fcitx_trigger_state_new(mode: u8) -> *mut VinpstFcitxTriggerState {
    catch_unwind(|| {
        let Some(mode) = trigger_mode(mode) else {
            return ptr::null_mut();
        };
        Box::into_raw(Box::new(VinpstFcitxTriggerState {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_trigger_state_free(state: *mut VinpstFcitxTriggerState) {
    if !state.is_null() {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_trigger_state_dispatch(
    state: *mut VinpstFcitxTriggerState,
    event: *const VinpstFcitxTriggerEventView,
    action_out: *mut u8,
) -> u8 {
...
mod tests {
    use super::{
        ...
        TRIGGER_EVENT_RELEASE, TRIGGER_EVENT_SET_MODE, VinpstFcitxTriggerEventView,
        vinpst_fcitx_trigger_state_dispatch, vinpst_fcitx_trigger_state_free,
        vinpst_fcitx_trigger_state_new,
    };

    unsafe fn dispatch(
        state: *mut super::VinpstFcitxTriggerState,
        event: VinpstFcitxTriggerEventView,
    ) -> Option<u8> {
        let mut action = u8::MAX;
        // SAFETY: Test callers pass a live state and local readable/writable values.
        let success = unsafe {
            vinpst_fcitx_trigger_state_dispatch(state, &raw const event, &raw mut action)
        };
    ...
    fn dispatches_complete_hold_lifecycle() {
        ...
        unsafe {
            let state = vinpst_fcitx_trigger_state_new(1);
            ...
            vinpst_fcitx_trigger_state_free(state);
        }
    }

    #[test]
    fn set_mode_and_invalid_events_are_atomic() {
        assert!(vinpst_fcitx_trigger_state_new(9).is_null());
        // SAFETY: The handle is live for every call and freed exactly once.
        unsafe {
            let state = vinpst_fcitx_trigger_state_new(1);
            assert_eq!(
            ...
            let invalid = VinpstFcitxTriggerEventView {
                kind: TRIGGER_EVENT_PRESS,
                value: 9,
                flag: 0,
                now_ns: 0,
            ...
            let mut action = 77;
            assert_eq!(
                vinpst_fcitx_trigger_state_dispatch(state, &raw const invalid, &raw mut action,),
                0,
            );
            assert_eq!(action, 77);
            assert_eq!(
