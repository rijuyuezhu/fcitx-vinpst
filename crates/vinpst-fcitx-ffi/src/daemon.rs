...
use vinpst_fcitx_dbus::{DaemonClient, DaemonOperation, DaemonResponse};

use crate::{
    ffi_string::{VinpstFcitxStringView, string_view, text_input},
    menu_controller::{
        VinpstFcitxAsrMenuController, VinpstFcitxSceneMenuController, asr_controller_mut,
        scene_controller_mut,
    },
};

/// Opaque blocking daemon client.
pub struct VinpstFcitxDaemonClient {
    pub(crate) client: DaemonClient,
}

/// Opaque Rust-owned UTF-8 string.
pub struct VinpstFcitxOwnedString {
    value: String,
}
...
pub struct VinpstFcitxAsrTargetView {
    /// ASR provider id.
    pub provider: VinpstFcitxStringView,
    /// Provider-specific model value.
    pub model: VinpstFcitxStringView,
}

impl VinpstFcitxAsrTargetView {
...
struct ErrorOut(*mut *mut VinpstFcitxOwnedString);

impl ErrorOut {
    unsafe fn new(output: *mut *mut VinpstFcitxOwnedString) -> Self {
        if !output.is_null() {
...
fn boxed_string(value: String) -> *mut VinpstFcitxOwnedString {
    Box::into_raw(Box::new(VinpstFcitxOwnedString { value }))
}

fn call(
    client: &VinpstFcitxDaemonClient,
...
unsafe fn bool_call(
    client: *const VinpstFcitxDaemonClient,
    operation: DaemonOperation,
    first: &str,
    second: &str,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_daemon_client_connect(
    error_out: *mut *mut VinpstFcitxOwnedString,
) -> *mut VinpstFcitxDaemonClient {
    catch_unwind(AssertUnwindSafe(|| {
        // SAFETY: Forwarded from this function's caller contract.
        let errors = unsafe { ErrorOut::new(error_out) };
        match DaemonClient::connect_session() {
            Ok(client) => Box::into_raw(Box::new(VinpstFcitxDaemonClient { client })),
            Err(error) => {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_daemon_client_free(client: *mut VinpstFcitxDaemonClient) {
    if !client.is_null() {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_daemon_client_get_status(
    client: *const VinpstFcitxDaemonClient,
    error_out: *mut *mut VinpstFcitxOwnedString,
) -> *mut VinpstFcitxOwnedString {
    catch_unwind(AssertUnwindSafe(|| {
        // SAFETY: Forwarded from this function's caller contract.
        let errors = unsafe { ErrorOut::new(error_out) };
        // SAFETY: Forwarded from this function's caller contract.
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_daemon_client_refresh_scene_menu_controller(
    client: *const VinpstFcitxDaemonClient,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_daemon_client_set_active_scene(
    client: *const VinpstFcitxDaemonClient,
    controller: *mut VinpstFcitxSceneMenuController,
    scene_data: *const u8,
    scene_len: usize,
...
/// Both handles must be live and `error_out` writable when non-null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_daemon_client_refresh_asr_menu_controller(
    client: *const VinpstFcitxDaemonClient,
...
/// All non-null pointers must satisfy the declared readable/writable lengths.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_daemon_client_set_active_asr_target(
    client: *const VinpstFcitxDaemonClient,
    target: *const VinpstFcitxAsrTargetView,
    persisted_out: *mut u8,
    error_out: *mut *mut VinpstFcitxOwnedString,
) -> u8 {
    u8::from(
...
/// A non-null pointer must be a live handle returned by this crate.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_owned_string_free(value: *mut VinpstFcitxOwnedString) {
    if !value.is_null() {
        let _ = catch_unwind(AssertUnwindSafe(|| {
            // SAFETY: Forwarded from this function's caller contract.
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_owned_string_view(
    value: *const VinpstFcitxOwnedString,
    view_out: *mut VinpstFcitxStringView,
) -> u8 {
    crate::ffi_catch(0, || {
        if view_out.is_null() {
            return 0;
...
mod tests {
    ...
    use vinpst_fcitx_core::{AsrDisplaySnapshot, SceneSnapshot};
    use vinpst_fcitx_dbus::DaemonResponse;

    use super::{
        boxed_string, expect, take_asr_display, take_scene, take_text,
        vinpst_fcitx_owned_string_free, vinpst_fcitx_owned_string_view,
    };
    ...
    fn exposes_owned_strings() {
        ...
        unsafe {
            for text in ["recording", "broken"] {
                ...
                let mut view = VinpstFcitxStringView {
                    data: ptr::null(),
                    len: 0,
                };
                assert_eq!(vinpst_fcitx_owned_string_view(value, &raw mut view), 1);
                assert_eq!(bytes(view), text.as_bytes());
                vinpst_fcitx_owned_string_free(value);
            }
        }
    }
