...
use vinpst_fcitx_core::{AsrDisplaySnapshot, AsrDisplaySnapshotItem, MenuFilterState};

use super::{
    VinpstFcitxMenuProjection, VinpstFcitxMenuProjectionView, VinpstFcitxProjectedMenuItemView,
    vinpst_fcitx_menu_projection_free, vinpst_fcitx_menu_projection_item_view,
    vinpst_fcitx_menu_projection_view,
};
use crate::{
    ffi_string::VinpstFcitxStringView,
    menu::{boxed_menu_session, vinpst_fcitx_menu_session_free},
    menu_controller::{
        VinpstFcitxAsrMenuController, VinpstFcitxAsrMenuTextView, boxed_asr_controller,
...
unsafe fn menu_projection(
    ...
    filter: *const crate::menu::VinpstFcitxMenuSession,
    ...
    unsafe { vinpst_fcitx_asr_menu_controller_projection_new(controller, filter, &raw const text) }
}
...
fn filter(query: &str) -> *mut crate::menu::VinpstFcitxMenuSession {
...
fn empty_string_view() -> VinpstFcitxStringView {
    VinpstFcitxStringView {
        data: ptr::null(),
        len: 0,
    }
...
fn projects_localized_rows_and_effective_label_from_controller() {
    ...
    unsafe {
        ...
        let mut summary = VinpstFcitxMenuProjectionView {
            summary: empty_string_view(),
            item_count: 0,
        };
        assert_eq!(
            vinpst_fcitx_menu_projection_view(projection, &raw mut summary),
            1,
        );
        ...
        let mut item = VinpstFcitxProjectedMenuItemView {
            label: empty,
            control_kind: 0,
            control_first: empty,
        ...
        assert_eq!(
            vinpst_fcitx_menu_projection_item_view(projection, 0, &raw mut item),
            1,
        );
        ...
        vinpst_fcitx_menu_session_free(filter);
...
fn invalid_localized_fragment_rejects_projection() {
    ...
    unsafe {
        ...
        let projection =
            vinpst_fcitx_asr_menu_controller_projection_new(controller, filter, &raw const text);
        assert!(projection.is_null());
        vinpst_fcitx_menu_session_free(filter);
        vinpst_fcitx_asr_menu_controller_free(controller);
    }
