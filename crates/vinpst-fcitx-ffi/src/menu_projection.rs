...
use vinpst_fcitx_core::{MenuControl, ProjectedMenuItem};

use crate::ffi_string::{VinpstFcitxStringView, string_view};

/// Opaque finalized menu projection shared by Scene and ASR menus.
pub struct VinpstFcitxMenuProjection {
...
pub struct VinpstFcitxMenuProjectionView {
    /// Fully rendered current-selection summary.
    pub summary: VinpstFcitxStringView,
...
#[repr(C)]
#[derive(Debug, Clone, Copy)]
pub struct VinpstFcitxProjectedMenuItemView {
    /// Fully rendered label.
    pub label: VinpstFcitxStringView,
    /// Stable `VINPST_FCITX_MENU_CONTROL_*` value.
    pub control_kind: u8,
    /// Scene id or ASR provider id.
    pub control_first: VinpstFcitxStringView,
    /// Empty for scenes, model value for ASR targets.
    pub control_second: VinpstFcitxStringView,
    /// User-visible base label for notifications.
    pub control_label: VinpstFcitxStringView,
}

const MENU_CONTROL_SET_ACTIVE_SCENE: u8 = 1;
const MENU_CONTROL_SET_ACTIVE_ASR_TARGET: u8 = 2;

pub(crate) fn projected_item_view(item: &ProjectedMenuItem) -> VinpstFcitxProjectedMenuItemView {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_menu_projection_free(
    projection: *mut VinpstFcitxMenuProjection,
) {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_menu_projection_view(
    projection: *const VinpstFcitxMenuProjection,
    view_out: *mut VinpstFcitxMenuProjectionView,
) -> u8 {
    crate::ffi_catch(0, || {
        if view_out.is_null() {
            return 0;
        ...
        unsafe {
            view_out.write(VinpstFcitxMenuProjectionView {
...
/// `projection` must be live and `view_out` writable.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vinpst_fcitx_menu_projection_item_view(
    projection: *const VinpstFcitxMenuProjection,
    index: usize,
    view_out: *mut VinpstFcitxProjectedMenuItemView,
) -> u8 {
    crate::ffi_catch(0, || {
        if view_out.is_null() {
            return 0;
