...
use crate::LiveVinputModelMetadata;

/// Metadata file materialized inside every managed model directory.
pub const INSTALLED_MODEL_METADATA_FILE: &str = "vinput-model.json";
...
pub struct InstalledModelInfo {
    ...
    pub metadata: LiveVinputModelMetadata,
...
///
/// Rust CLI installs currently use `<root>/<managed-name>/vinput-model.json`.
/// The legacy project uses `<root>/<engine>/<name>/vinput-model.json`, with a
...
fn load_installed_model_info_with_id(
    ...
    let metadata =
        serde_json::from_str::<LiveVinputModelMetadata>(&metadata_text).map_err(|source| {
...
mod tests {
    ...
    fn invalid_metadata_reports_the_concrete_path() {
        ...
        assert!(error.to_string().contains("broken/vinput-model.json"));
    }
}
