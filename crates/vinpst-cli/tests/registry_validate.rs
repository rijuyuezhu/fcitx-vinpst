...
use common::{assert_json_success, vinpst_command, workspace_file, write_temp_json};
use vinpst_registry::RegistryIndex;

fn write_temp_registry(contents: &str) -> std::path::PathBuf {
    write_temp_json("vinpst-registry", contents)
}
fn write_temp_config(contents: &str) -> std::path::PathBuf {
    write_temp_json("vinpst-plan-config", contents)
}
...
fn registry_validate_accepts_committed_sample_fixture() {
    ...
    let output = vinpst_command()
        ...
        .arg(&path)
        .output()
        .expect("run vinpst registry validate on sample fixture");
...
fn registry_plan_accepts_committed_sample_fixture() {
    ...
    let output = vinpst_command()
        ...
        .arg("--summary-only")
        .output()
        .expect("run vinpst registry plan on sample fixture");
...
fn registry_validate_prints_summary_for_valid_index() {
    ...
    let output = vinpst_command()
        ...
        .arg(&path)
        .output()
        .expect("run vinpst registry validate");
...
fn registry_plan_prints_assets_with_resolved_urls() {
    ...
    let output = vinpst_command()
        ...
        .arg(&path)
        .output()
        .expect("run vinpst registry plan");
    fs::remove_file(&path).expect("remove temporary registry fixture");
...
fn registry_install_plan_prints_targets_and_checksum_policy() {
    ...
    let output = vinpst_command()
        ...
        .arg(&path)
        .args(["--target-root", "/tmp/vinpst-assets"])
        .output()
        .expect("run vinpst registry install-plan");
    fs::remove_file(&path).expect("remove temporary registry fixture");

    let value = assert_json_success(output, "registry install plan");
    assert_eq!(value["ok"], true);
    assert_eq!(value["target_root"], "/tmp/vinpst-assets");
    assert_eq!(value["asset_count"], 2);
    assert_eq!(value["known_size_bytes"], 5);
    ...
    assert_eq!(
        ...
        "/tmp/vinpst-assets/models/m.tar"
    );
...
fn registry_install_plan_rejects_invalid_config_mirrors() {
    ...
    let output = vinpst_command()
        .args(["registry", "install-plan"])
        .arg(&registry_path)
        .args(["--target-root", "/tmp/vinpst-assets", "--config"])
        .arg(&config_path)
        .output()
        .expect("run vinpst registry install-plan");
    fs::remove_file(&registry_path).expect("remove temporary registry fixture");
