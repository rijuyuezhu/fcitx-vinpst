...
- Native `sherpa-onnx` is feature-gated and supports offline SenseVoice/Qwen3 ASR/Moonshine v1 plus online transducer/Zipformer2 CTC layouts. SenseVoice, Qwen3 ASR, and Moonshine v1 pass real WAV smokes with offline Silero VAD active; Zipformer2 CTC also has a real WAV smoke with the 200 ms recognizer warmup, while transducer construction remains metadata/feature-build tested. Legacy endpoint rule defaults and metadata overrides are forwarded to the official runtime. Timeout capability is explicit: command helpers enforce configured deadlines, while native synchronous decode reports configured values as unsupported/diagnostic-only. Prepare-before-swap warm reload is implemented; a non-blocking reload worker, Dolphin, and Paraformer are not implemented yet.
