...
`scripts/tests/asr/run-openai-compatible-asr-fixture-smoke.sh` deterministically validates multipart parsing, Bearer authentication, WAV shape/signal, model/language/prompt matching, response handling, and evidence redaction. `scripts/live/niri/run-ime-fcitx-remote-asr-live.sh` proves the real Fcitx/PipeWire path against that independent loopback process and restores the original streaming backend. The fixture proves protocol/runtime integration, not a third-party hosted service or its network and credential operations.
...
- Command and remote ASR providers are runtime-wired. Remote providers are buffered/final-only OpenAI-compatible HTTP clients; loopback multipart/Bearer recognition and prepare-failure preservation are live-proven, while real hosted-service DNS/TLS/proxy/rate-limit/credential behavior remains unproven.
