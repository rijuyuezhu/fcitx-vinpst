...
The CLI preserves legacy command-provider script editing as the separate `provider edit-script` operation so the existing Rust `provider edit` config-mutation surface remains stable. It resolves exact installed ids before explicit registry short ids, rejects local/remote providers, treats a path-like command as the first candidate and then scans arguments, opens only an existing regular file, and never mutates config. Editor selection is explicit `--editor`, `$VINPUT_PROVIDER_EDITOR`, `$VISUAL`, `$EDITOR`, then `vi`; dry-run resolves the entire plan without launching a process.
...
5. `timeout_ms` is enforced by the HTTP request deadline, non-success HTTP responses retain their response body in the backend error, and warmup only creates and cancels a session without contacting the endpoint;
...
- Command and remote ASR providers are runtime-wired. Remote providers are buffered/final-only OpenAI-compatible HTTP clients; loopback multipart/Bearer recognition and prepare-failure preservation are live-proven, and deterministic process tests cover proxy/`NO_PROXY`, 429/503, timeout, self-signed TLS rejection, DNS failure, connection refusal, and credential redaction. Real hosted-provider and production credential operations remain unproven.
