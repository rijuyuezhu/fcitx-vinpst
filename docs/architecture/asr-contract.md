...
- The retained frontend has a Fcitx D-Bus monitor for `StatusChanged(s)` and `RecognitionPartial(s)`. The monitor can be created before the daemon owns its well-known name and follows owner replacement without trusting arbitrary senders. For addon-owned sessions, partial text takes precedence over localized recording/commanding/recognizing fallback in the active input-context preedit. A temporary-HOME native online-transducer smoke proves that this partial reaches a concrete Fcitx test `InputContext` before stop, while `ime-fcitx-native-live` proves partial input-panel updates and final synchronous-stop commit in one real Fcitx client application. Idle/error status clears frontend state. Representative GUI-toolkit rendering remains separate live work.
...
`MockAsrBackend` can attach a shared `MockAsrAudioLog` for deterministic tests. The log records each `push_audio` or `push_pcm` call, including sample length and optional `PcmSpec` metadata. Runtime tests use it to prove 800/800/tail chunk delivery, input metadata preservation, and no stop-time replay. `SourceAudioRecorder` additionally proves begin-time callback delivery and cached stop retrieval for deterministic source-backed streaming. A real session-bus integration test proves that a partial signal arrives before `StopRecording` and is not repeated at stop, while the native addon activation smoke proves the same signal reaches concrete Fcitx preedit before stop. Native online transducer and Zipformer2 CTC prove both streaming recognizer/runtime layouts separately; `ime-fcitx-native-live` proves one real acoustic PipeWire/Fcitx application path.
