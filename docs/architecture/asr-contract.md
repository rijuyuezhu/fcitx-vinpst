...
`just openai-compatible-asr-fixture-smoke` deterministically validates multipart parsing, Bearer authentication, WAV shape/signal, model/language/prompt matching, response handling, and evidence redaction. `just ime-fcitx-remote-asr-live` proves the real Fcitx/PipeWire path against that independent loopback process and restores the original streaming backend. The fixture proves protocol/runtime integration, not a third-party hosted service or its network and credential operations.
...
The model-free `just user-ime-activation-owner-smoke` pins the activation ordering contract: the CLI performs a real service method call before reading `GetNameOwner`, so the first `daemon status` result already reports a daemon that was activated by that query. The model-backed `just user-ime-sherpa-native-activation-smoke` then proves the installed wrapper, copied libraries, generated configuration, D-Bus activation, installed-daemon owner, `StartRecording`/`StopRecording`, and exact recognition result in one temporary-HOME path. `just sherpa-online-transducer-user-activation-smoke` supplies the proven English online-transducer fixture. `just sherpa-online-transducer-user-frontend-smoke` moves the first activation call to the retained C++ `FrontendBridge`, verifies its typed `Commit` outcome, payload commit text, and raw candidate, then applies the real outcome through `OutcomeSink` and pins the `preedit -> clear candidates -> clear preedit -> commit` ordering used by the addon. `just sherpa-online-transducer-user-addon-smoke` advances one layer further: `FcitxVinputAddon::ApplyTriggerAction` is the first activation client and its normal Start/Stop actions run through the production `ApplyBridgeOutcomeToInputContext` implementation and a concrete test subclass of `fcitx::InputContext`, where `commitStringImpl` captures the exact transcript. `just sherpa-online-transducer-user-command-addon-smoke` covers selected-text command mode without a configured adapter. `CommandTextProcessor` preserves the selected text as the default `raw` candidate and includes the recognized command as an `asr` candidate; `FrontendBridge` opens a candidate menu whenever command mode has multiple candidates, while ordinary recognition retains the existing multiple-LLM-only rule. The smoke selects the ASR candidate through the real Fcitx candidate object, captures `deleteSurroundingTextImpl`, and then captures the replacement commit. A separate CTest drives the same production sink directly and pins genuinely empty preedit clearing. The opt-in `ime-fcitx-virtual-source-live` gate crosses the real session boundary with an isolated PipeWire sink/source, verifies non-silent PCM before changing the live profile, then proves partial input-panel updates, final commit, selected-text deletion/replacement, focus handoff, owner loss, and same-provider reload in real Fcitx clients without physical audio devices. The companion `ime-fcitx-physical-microphone-live` gate verifies that the default capture target is a physical ALSA source and requires real partials plus a final commit without playback injection; representative GUI-toolkit and additional-device breadth remain outside these proofs.
...
`MockAsrBackend` can attach a shared `MockAsrAudioLog` for deterministic tests. The log records each `push_audio` or `push_pcm` call, including sample length and optional `PcmSpec` metadata. Runtime tests use it to prove 800/800/tail chunk delivery, input metadata preservation, and no stop-time replay. `SourceAudioRecorder` additionally proves begin-time callback delivery and cached stop retrieval for deterministic source-backed streaming. A real session-bus integration test proves that a partial signal arrives before `StopRecording` and is not repeated at stop, while the native addon activation smoke proves the same signal reaches concrete Fcitx preedit before stop. Native online transducer and Zipformer2 CTC prove both streaming recognizer/runtime layouts separately; `ime-fcitx-virtual-source-live` proves one real PipeWire/Fcitx application path without physical audio devices.
