...
Live partials arrive through the Fcitx D-Bus monitor, override status fallback in preedit, and final commit remains driven by the synchronous stop reply. Current deterministic evidence covers owner loss, external-session reconciliation, background ASR reload failures, menu behavior, i18n, and outcome application. Real desktop PipeWire and application behavior remain outside the architecture contract and are tracked in `docs/migration/e2e-replication-plan.md`.

Do not replace the Fcitx5 addon with a Rust addon until mature Rust bindings and deployment integration are validated. Packaging and service artifacts remain separate from daemon, registry, and frontend logic.
...
- Raw HTTP/WebSocket code in `daemon/remote`: the deterministic protocol core, Axum runtime, and normal-daemon reload/shutdown ownership are implemented; add redacted endpoint diagnostics and live cross-device proof rather than porting raw socket code.
