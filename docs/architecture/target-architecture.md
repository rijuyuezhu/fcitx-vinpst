# Target architecture

This architecture keeps the risky C++ Fcitx5 addon boundary thin while moving daemon/backend logic to Rust crates with explicit test seams.

## Top-level shape

```text
fcitx-vinput-rs/
  crates/
    vinput-protocol     # stable D-Bus/JSON ABI shared by all components
    vinput-config       # config schema, migration, normalization, validation
    vinput-audio        # PipeWire capture and pure PCM transforms
    vinput-asr          # ASR traits, mock backend, command backend, sherpa-onnx backend
    vinput-text         # scene prompts, text adapters, command-mode text transforms
    vinput-registry     # registry metadata, download, safe extraction, materialization
    vinput-daemon       # async runtime, D-Bus service, orchestration actors
    vinput-cli          # clap CLI over protocol/config/daemon APIs
  cpp/
    fcitx5-addon        # retained thin AddonInstance frontend bridge
    gui-qt              # optional retained GUI until a Rust UI decision is made
  data/
  docs/
```

The current workspace already has the pure protocol/config/audio/ASR/text/registry crates plus `vinput-daemon` and `vinput-cli`. Backend implementations can keep landing behind those seams without changing the top-level crate boundaries.

## Runtime actors

```text
Fcitx5 addon (C++)
  └─ D-Bus methods/signals using vinput-protocol ABI
      └─ vinput-daemon::dbus
          └─ Runtime actor
              ├─ Audio capture task          -> vinput-audio
              ├─ ASR session task            -> vinput-asr
              ├─ Postprocess task            -> vinput-text
              ├─ Adapter supervisor task     -> vinput-text / vinput-daemon
              ├─ Remote text service task    -> vinput-daemon::remote
              └─ Registry/install helpers    -> vinput-registry
```

## State machine

The daemon should make state transitions explicit and testable:

```text
Idle
  ├─ StartRecording / StartCommandRecording
  ▼
Recording
  ├─ RecognitionPartial*      # streaming backends only
  ├─ StopRecording
  ▼
Inferring
  ├─ ASR final/error
  ▼
Postprocessing?               # scene/LLM/command mode
  ├─ RecognitionResult / Notification
  ▼
Idle
```

Every transition should have a unit test before it is wired to D-Bus or PipeWire.

## Compatibility contracts

`vinput-protocol` owns the stable contract:

- bus name: `org.fcitx.Vinput`
- object path: `/org/fcitx/Vinput`
- interface: `org.fcitx.Vinput.Service`
- status strings: `idle`, `recording`, `inferring`, `postprocessing`, `error`
- recognition result JSON: `{ "commit_text": string, "candidates": [{ "text": string, "source": string }] }`
- ASR backend state JSON fields matching the original frontend expectations
- Config file baseline and diagnostics behavior: see `docs/architecture/config-contract.md`
- Registry metadata and planning behavior: see `docs/architecture/registry-contract.md`

Any change to this crate must include compatibility tests.


## Active E2E acceleration target

The current implementation phase prioritizes a usable product spine over further broad refactor work. The retained thin C++ Fcitx5 bridge now talks to the Rust daemon over the existing protocol boundary, commits mock or configured recognition results, and owns frontend-side preedit, result candidates, selected-text replacement, the minimal scene menu, and an installed-model-aware ASR menu. Right Shift opens scene candidates and F8 opens ASR model candidates by default; keyboard, paging, digit, enter, escape, and mouse candidate selection stay in C++. The four normal/command/scene/ASR triggers plus `PagePrevKeys` and `PageNextKeys` are persistent Fcitx `KeyList` options in the legacy `conf/vinput.conf` schema; both scene and ASR menus read the current paging lists, whose defaults include main-keyboard and keypad PageUp/PageDown. Both menus also share a slash-activated filter model with ASCII case-insensitive multi-term AND matching, UTF-8 Backspace, Ctrl+W/Ctrl+U editing, two-stage Escape, and digit-as-query behavior while filtering. Static menu/config/result labels use the `fcitx5-vinput` gettext domain through `fcitx::registerDomain` and `translateDomain`; the build-tree catalog can be selected for tests, the installed catalog lives under the standard locale directory, and untranslated locales retain English message ids. Registry-backed model titles remain a Rust/registry-data concern rather than a C++ gettext concern: installation persists the full registry id plus the selected locale title in `vinput-model.json`, `GetAsrDisplayMenuState` resolves it from daemon locale preferences, and old/unmanaged installs fall back to stable ids without network access. Local frontend errors and scene/ASR switch confirmations pass through a small notification presenter that pins the legacy information/warning/error icons, 3000/5000 ms timeouts, notifications-addon dispatch, and stderr fallback; error outcomes still update preedit. A separate Fcitx D-Bus monitor subscribes to `StatusChanged(s)`, `RecognitionPartial(s)`, and `DaemonNotification(ssss)`, while Fcitx `ServiceWatcher` provides race-free initial owner discovery and owner-change delivery. During addon-owned recording, partial text overrides localized status fallback in the active input context preedit; unexpected idle/error status resets the frontend session, while final commit remains driven by the synchronous stop reply. Error-like notification payloads reset frontend recording state before presentation, while raw informational payloads remain non-disruptive. Daemon startup absence is silent, but owner loss during addon-owned recording or status-only recovery invalidates the synchronous client, clears the frontend state, and presents the localized unavailable error. Trigger-time `GetStatus` reconciliation lets explicit idle continue to Start, lets the normal trigger adopt and stop an externally started recording, and maps external recording/inferring/postprocessing state to tracked preedit instead of issuing a conflicting Start. The Rust daemon emits current-generation background ASR reload failures through this path; broader notification categories and live desktop signal/preedit presentation remain incomplete. `TriggerMode=Tap|Hold|Both` is also implemented with the legacy default `Both`. A pure frontend state machine pins the 80 ms debounce, 300 ms hold threshold, and 500 ms release-tail delay, while the addon owns only Fcitx timers, safe input-context references, and actual start/stop calls. The addon implements `reloadConfig`, `getConfig`, `setConfig`, and `save`, while environment variables remain temporary single-key overrides. Saving known fields merges into the existing `RawConfig`, preserving not-yet-ported legacy fields. Scene enumeration/persistence stays in Rust through `GetSceneState` and `SetActiveScene`; ASR target/effective/loading state, installed-model discovery, localized display rows, and atomic provider/model selection stay in Rust through `GetAsrDisplayMenuState` and `SetActiveAsrTarget`; the older target-menu method remains available as a compatibility extension. Normal recording refreshes the active scene before start so a persisted selection does not depend on opening the menu first. Backend logic should stay in Rust crates; the frontend should own only Fcitx API integration, menu/status/preedit/candidate UI, text context, and the bus bridge.

See `docs/migration/e2e-port-plan.md` for the active Rust-vs-legacy gap list and execution phases.

## Frontend and packaging boundary

T6 uses a retained C++ Fcitx5 frontend bridge that talks to the Rust daemon over the existing `vinput-protocol` D-Bus ABI. The bridge owns Fcitx API integration, menus, preedit/status presentation, selected-text collection, command-mode selected-text replacement, and frontend-side cleanup. Backend logic, ASR/text processing, registry operations, and runtime state must stay in Rust crates and the daemon.

Do not replace the Fcitx5 addon with a Rust addon until mature Rust bindings and deployment integration have been verified separately. Packaging/service install artifacts remain future work and must not be hidden inside daemon, registry, or frontend logic.

## TDD migration order

1. **Protocol/config locked baseline**
   - Add golden JSON tests for existing daemon/frontend payloads.
   - Add config migration tests before editing defaults.

2. **D-Bus daemon shell**
   - Add a `zbus` service that exposes legacy methods/signals.
   - Test under `dbus-run-session` using a Rust proxy.
   - Keep mock runtime behind the service first.

3. **Runtime state machine**
   - Replace the demo runtime with an actor that accepts typed commands.
   - Test busy/error/cancel/reload races without audio or ASR.

4. **Audio and ASR seams**
   - Port pure PCM transforms first.
   - Add `AsrBackend`/`RecognitionSession` trait with mock implementation.
   - Add PipeWire and sherpa-onnx behind feature/integration tests.

5. **Postprocess and command mode**
   - Port prompt rendering and command-mode behavior with fixture tests.
   - Mock LLM adapter HTTP/process edges before real adapter supervision.

6. **Registry/CLI/GUI/addon tightening**
   - Port registry parsing/download with safe extraction tests.
   - Rebuild CLI commands against typed crates.
   - Reduce C++ addon to Fcitx API, menus, preedit, and D-Bus bridge.
   - Defer GUI rewrite unless Qt maintenance becomes worse than a Rust UI.

## What not to port mechanically

- Raw HTTP/WebSocket code in `daemon/remote`: replace with a Rust HTTP/WebSocket stack.
- Generic path/file/process/string utilities: prefer well-maintained Rust crates.
- C++ daemon poll loop: replace with structured async tasks and explicit shutdown.
- Ad-hoc JSON parsing: use typed serde models and golden fixtures.
