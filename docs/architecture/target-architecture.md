...
Live partials arrive through the Fcitx D-Bus monitor, override status fallback in preedit, and final commit remains driven by the synchronous stop reply. Deterministic evidence covers owner loss, external-session reconciliation, background ASR reload failures, menu behavior, i18n, and outcome application. The opt-in `ime-fcitx-native-live` gate additionally proves one real Fcitx client path through acoustic PipeWire capture, partial input-panel updates, final commit, and command replacement. Broader GUI-toolkit and failure/reload behavior remains tracked in `docs/migration/e2e-replication-plan.md`.
