...
The current recipe proves package construction, extracted-runtime behavior, and isolated pacman install/upgrade/same-version-rollback/uninstall transactions. Rollback across versions with incompatible config or state, running-service handoff during upgrade/removal, stale-owner cleanup, repository publication, package signing, and live installed-desktop validation remain separate release-readiness work.
