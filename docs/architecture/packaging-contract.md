...
The D-Bus activation file carries the same `Exec=` fallback plus `SystemdService=vinput-daemon.service`. The installed systemd user unit starts the daemon with `--exit-when-executable-replaced`, `Restart=on-failure`, and a one-second restart delay. Once that unit has started a daemon, a later atomic package replacement changes the executable inode; the daemon detects the change, shuts down owned services, exits unsuccessfully, and systemd starts the new executable. The direct D-Bus `Exec=` fallback remains available without automatic restart. The daemon discovers `$XDG_CONFIG_HOME/fcitx-vinput/config.json` or `$HOME/.config/fcitx-vinput/config.json`; the packaged reference config is not copied over user state during upgrades.
...
- `post_upgrade` explains that systemd-backed daemons started by the new unit restart themselves after executable replacement, while the first upgrade from an older unit and direct-activation owners may still require `vinput daemon handoff` plus `fcitx5 -r`;
