...
The current recipe proves package construction, extracted-runtime behavior, and isolated pacman install/upgrade/same-version-rollback/uninstall transactions. `vinput daemon status` now detects a running D-Bus owner from a different executable path or an executable inode unlinked by package replacement and gives an explicit restart recommendation without mutating the service. Automatic running-service handoff during upgrade/removal, rollback across versions with incompatible config or state, destructive stale-owner cleanup, repository publication, package signing, and live installed-desktop validation remain separate release-readiness work.
