...
- `just arch-package-transaction-smoke` reuses the `pkgrel=1` and `pkgrel=2` archives under a fakeroot-isolated pacman root. It proves package database registration, file ownership and integrity, install, upgrade, same-version `pkgrel=2` to `pkgrel=1` rollback, and removal. The package carries a pre-remove hook and the installed `/usr/lib/fcitx-vinput/package-remove-handoff` helper. A pre-existing `$HOME/.config/fcitx-vinput/config.json` sentinel remains byte-identical throughout, and the package owns no `/etc` or `/home` paths.
