...
- `just arch-package-transaction-smoke` reuses the `pkgrel=1` and `pkgrel=2` archives under a fakeroot-isolated pacman root. It proves package database registration, file ownership and integrity, install, upgrade, and removal. A pre-existing `$HOME/.config/fcitx-vinput/config.json` sentinel remains byte-identical throughout, and the package owns no `/etc` or `/home` paths.

The current recipe proves package construction, extracted-runtime behavior, and isolated pacman install/upgrade/uninstall transactions. Rollback across incompatible versions, running-service handoff during upgrade/removal, stale-owner cleanup, repository publication, package signing, and live installed-desktop validation remain separate release-readiness work.
