...
- `installed.rs`: shared installed-model discovery, typed `vinput-model.json` loading, and regular-file inventory for both the current flat Rust managed layout and the legacy two-level engine/model layout;
