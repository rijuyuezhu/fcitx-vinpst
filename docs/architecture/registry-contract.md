...
- `script.rs`: live `registry/adapters.json` parsing, legacy-compatible managed script paths, mirror-backed script publication, executable permissions, environment defaults, and guarded adapter materialization;
...
The live adapter CLI uses the current upstream script registry:
...
Omitting `--registry` fetches `registry/adapters.json` from configured mirrors. A real install downloads the script into `$XDG_DATA_HOME/fcitx-vinput/adapters` by default, marks it executable, and writes configuration through the same output/in-place/backup policy as other config mutations. Dry-run performs registry, path, overwrite, and config validation without downloading or writing.
