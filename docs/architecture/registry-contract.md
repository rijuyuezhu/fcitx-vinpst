...
Omitting `--registry` fetches `registry/providers.json` or `registry/adapters.json` from configured mirrors. Real installs download into `$XDG_DATA_HOME/fcitx-vinput/providers` or `$XDG_DATA_HOME/fcitx-vinput/adapters`, mark the script executable, and write configuration through the same output/in-place/backup policy as other config mutations. Dry-run performs registry, path, protocol, overwrite, and config validation without downloading or writing.
