...
- `openai.rs`: OpenAI-compatible request building, provider selection, candidate parsing, injected transport seams, and the cancellable synchronous reqwest transport backed by a dedicated async worker;
...
Response fields are `payload` for a full `RecognitionPayload`, `text` for a simple final post-processed text, or `error` for a helper-level error. Response objects are strict: unknown fields, including the unpublished `failure` key, are rejected. Full payload responses are normalized with the same upstream-compatible rules as the D-Bus recognition payload. Empty or whitespace-only `text` is rejected as a missing final text response. Empty or whitespace-only `error` is ignored.
...
The default daemon constructor uses mock text processing for deterministic/demo operation. To exercise configured backends explicitly, run the daemon with `--configured-backends`. That path builds ASR from `asr.active_provider` and text post-processing from `llm.providers[]` through `ReqwestOpenAiCompatibleChatTransport` when providers are configured; when no providers are configured it falls back to command adapters from `llm.adapters[]`.

Prompt-file compatibility mirrors the legacy daemon: only literal `file:///absolute/path` URIs are accepted, the path is loaded only when it points to a regular file, and reads are capped at 256 KiB. Legacy double-brace interpolation accepts optional whitespace around variable names; unsupported variables are preserved verbatim. Plain `PromptTemplate` rendering keeps `{{context}}` empty for deterministic non-runtime tests, while OpenAI-compatible request builders can load the recent-input cache and inject the rendered context prefix. OpenAI-compatible request helpers preserve the legacy `extra_body` merge rule: provider-specific fields pass through, while `messages`, `stream`, and `response_format` are protected because they are required for the JSON candidates contract. Request diagnostics redact the HTTP auth header, remove URL userinfo/fragments, replace query values with `REDACTED`, and show only top-level body keys in `Debug`; the actual transport request retains its original URL, query values, headers, and JSON body. Non-success provider responses are reported by HTTP status only and their bodies are deliberately left unread; the shared 1 MiB UTF-8 body limit and post-header timeout diagnostics apply to successful responses.

The recent-input cache mirrors the legacy frontend/daemon split without moving filesystem or Unicode policy back into C++. The retained Fcitx addon forwards post-input-method commit events to a Rust-owned history tracker. User fragments preserve the legacy CJK/space joining rule, flush on sentence-ending punctuation or input-context changes, otherwise flush after five seconds of inactivity, and flush when their owning input context is destroyed or the addon exits. Recognition results are recorded before the generated Fcitx commit: an explicit `asr` candidate wins, otherwise the first non-empty `raw` candidate is treated as ASR while command mode excludes the originally selected text; a directly committed LLM result adds a separate `llm` entry, and an LLM candidate adds its entry only when the user selects it. Every generated non-cancel commit installs an exact one-shot suppression so the later `InputContextCommitString` event cannot duplicate it as `user`. The tracker reads the canonical user JSON config on load/reload to compute the maximum configured `scene.context_lines`, preserves the frozen every-100-writes truncation cadence when that maximum is nonzero, and treats config/cache maintenance failures as non-fatal context loss rather than recognition failure. Daemon-facing request builders continue to read raw non-empty JSONL lines and send the last `scene.context_lines` entries as context. The default cache path follows legacy XDG order: `XDG_CACHE_HOME/vinpst/context.jsonl`, then `$HOME/.cache/vinpst/context.jsonl`, then relative `vinpst/context.jsonl` when no base exists.
...
`OpenAiCompatibleTextProcessor` uses the scene provider when set and otherwise requires exactly one configured provider. `ReqwestOpenAiCompatibleChatTransport` preserves the synchronous processor seam while running async reqwest on a dedicated current-thread Tokio worker. The daemon shares one shutdown token across text-processor reloads, so SIGTERM/SIGINT and executable-replacement shutdown can drop an in-flight request or response-body future without waiting for the scene deadline; cancellation falls back to raw/selected+ASR text without emitting a provider failure warning. `scripts/tests/asr/run-openai-compatible-text-provider-fixture-smoke.sh` keeps only a short production `vinpst llm test` loopback boundary for authenticated request shape and result parsing; the larger proxy/TLS matrix is exercised once at the shared HTTP/remote-ASR boundary instead of duplicated for text. `scripts/tests/asr/run-provider-ca-rotation-smoke.sh` retains the cross-provider same-daemon CA replacement proof.
