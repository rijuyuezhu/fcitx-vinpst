...
`CommandTextProcessor` only dispatches a post-processing scene when exactly one command adapter is configured. `CommandTextRequest` serializes the effective scene timeout, including the scene default when the config omits it. `ProcessCommandTextRunner` delegates the parent boundary to the shared `vinpst-process` supervisor: each helper starts in its own Unix process group, dedicated readers drain stdout/stderr without pipe deadlock, and a watchdog sends `SIGKILL` to both the process group and direct child before returning a fixed timeout error. The watchdog starts immediately after spawn, so the deadline covers stdin writing, helper execution, and output recovery; in-memory request encoding happens before spawn. Stdout and stderr are each capped at 1 MiB; reaching byte 1,048,577 records a fixed stream-specific error, terminates the process group, and never includes captured output in the diagnostic. Tests prove a helper that ignores a 1 MiB stdin request still times out, a background descendant is no longer runnable, a helper can emit 256 KiB of stderr before a valid response, and oversized stdout/stderr are rejected promptly. With no adapters it returns `AdapterRequired`; with multiple adapters it returns `AmbiguousAdapter` instead of guessing. `OpenAiCompatibleTextProcessor` uses `SceneDefinition::provider_id` when set; without it, exactly one configured provider is required, zero providers return `AdapterRequired`, and multiple providers return `AmbiguousProvider`. Runtime code can use `RuntimeState::with_configured_backends` for configured ASR plus configured OpenAI-compatible providers or configured command text adapters, or `RuntimeState::with_configured_text` when ASR/audio seams are injected in tests. `ReqwestOpenAiCompatibleChatTransport` is the concrete blocking HTTP transport for the existing pure request/processor seam. Deterministic tests isolate it behind local servers, and `scripts/live/niri/run-ime-fcitx-external-text-provider-live.sh` proves the real configured-daemon/F10 path against independent loopback servers. Its failure phase returns HTTP 404 after real ASR and selected-text capture; Fcitx surfaces the operation error while committing nothing, deleting nothing, preserving the selected buffer byte-for-byte, returning the daemon to idle, and retaining the failing HTTP evidence. Its recovery phase installs a valid endpoint, repeats F10, checks Bearer auth without retaining the token, sends selected and raw ASR text to `/v1/chat/completions`, selects the JSON candidate, deletes the original surrounding text, and commits the exact provider result. A final phase clears both surrounding selection and the Wayland primary selection, proves the frontend rejects F10 with `Please select text first.` before recording or provider access, emits no commit/delete, keeps the daemon idle, and restores the prior primary selection plus local adapter/profile/backend. `scripts/tests/asr/run-openai-compatible-text-network-smoke.sh` separately drives the production `vinpst llm test` transport through plain-HTTP proxy routing, proxy-URL Basic authentication, `NO_PROXY` bypass, 429/503 bodies, separately classified request and response-body timeouts, self-signed TLS rejection, DNS failure, and connection refusal while proving that API and proxy credentials are absent from retained evidence and the CLI request plan redacts `Authorization` plus URL userinfo/query values/fragments. Explicit `llm test --dry-run` output still includes the synthetic request body and caller-supplied test text by design. The configured provider URL is the only request target: redirects are disabled, so any 3xx is handled as an ordinary HTTP failure and its `Location` is never contacted. A scene without `timeout_ms` uses the legacy 4000 ms request deadline; explicit positive values override it, and `llm test` reports the effective value. HTTP failure bodies replace exact configured Authorization/Bearer values with `<redacted>`; this known-value replacement does not claim general provider-response secret detection. Success and error bodies are read as UTF-8 with a shared 1 MiB limit before candidate parsing or diagnostic-body rendering; oversized bodies and invalid UTF-8 fail with fixed messages that contain neither body content nor a raw URL. The current reqwest feature set does not enable compressed-response decoding, so the limit applies to the body bytes exposed by the transport reader. The same deterministic gate adds a temporary PEM root through `SSL_CERT_FILE`, completes Basic-authenticated CONNECT tunnels through both plain-HTTP and TLS-protected HTTPS proxy endpoints to CA-signed HTTPS origins, and relays one local CA-signed TLS interception exchange while keeping built-in `WebPKI` roots and certificate verification enabled. The CONNECT fixture records no tunneled payload, and the interception fixture retains only TLS versions and byte counts rather than decrypted headers, request text, or credentials. `scripts/tests/asr/run-provider-ca-rotation-smoke.sh` keeps one configured text-processing daemon alive while the same CA file path is atomically replaced: the first CA succeeds, a mismatch fails without leaving an active session, and the newly signed origin succeeds under the same owner PID and provider endpoint. This is loopback HTTP application-error/recovery, frontend no-selection, and deterministic local network-semantics proof, not third-party cloud-service, PAC, NTLM/Kerberos, enterprise TLS-interception policy and certificate deployment, or production provider-credential lifecycle and CA distribution/revocation proof.
