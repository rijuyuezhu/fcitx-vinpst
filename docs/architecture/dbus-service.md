...
Live ASR `Error` events are already fatal to stop-time recognition payload construction, but immediate upstream-style `DaemonNotification` classification for an error observed while recording is not claimed by this slice. That behavior depends on the separate upstream `common/dbus/error_info.*` taxonomy and remains explicitly pending in the module-parity ledger rather than being folded into the recognition-contract audit.
