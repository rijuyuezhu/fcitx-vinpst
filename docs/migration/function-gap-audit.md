...
| Remote text service | Protocol/config core implemented and deterministic; HTTP/WebSocket runtime and daemon lifecycle integration missing |
...
4. **Remote parity:** settings, authentication, ownership, debounce, and Realtime-compatible event semantics are deterministic, but no HTTP/WebSocket server is wired into the daemon yet.
