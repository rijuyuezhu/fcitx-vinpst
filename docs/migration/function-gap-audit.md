...
| Real desktop normal dictation | Live-proven through a real Fcitx client with both an isolated PipeWire virtual source and the default physical ALSA Digital Microphone; both paths require streaming partials and a non-empty final commit, and GTK4 additionally passes three same-window/same-daemon cycles |
| Real desktop command dictation | Local adapter-backed surrounding-text deletion/replacement, loopback OpenAI-compatible HTTP-provider replacement, and zero-delete Wayland primary-selection fallback are live-proven; GTK3, GTK4, Qt6, Chromium, GNOME Text Editor, and kitty command paths also pass; Chromium command mode is PRIMARY-fallback proof because its page selection is not exposed as Fcitx surrounding text; GTK4 additionally passes three same-window/same-daemon replacement cycles | Broader cross-application and real cloud-provider proof remain |
...
| Distro packaging and upgrades | Partial: the checked Arch `x86_64` package, isolated install/upgrade/pkgrel rollback/removal transactions, signed release-gate inventory, candidate promotion, current/old-metadata handoff, private-session direct replacement, real user-systemd restart/restore, guarded removal preparation/busy rollback, and unsupported-future-schema refusal with byte-identical user config are complete. Production publication, an actual host package-installed upgrade, live production multi-user removal, production key operations, automatic cross-user old-upgrade invocation, and external-user regression remain. Real schema-migration rollback becomes applicable when schema 2 exists. See [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md). |
...
| User install | Temporary-HOME profiles, direct/systemd activation, native runtime bundle, checked package construction, current/old-metadata handoff, private-session direct replacement, real user-systemd restart/restore, guarded removal handoff/rollback, and future-schema refusal/preservation through package install/upgrade/pkgrel rollback/removal | Actual host package-installed upgrade, live production multi-user removal, automatic cross-user old-upgrade invocation, version selection, production repository/key policy, and live external-user policy; migration rollback waits for schema 2 |
...
1. **Application breadth:** GTK3, GTK4, Qt6, Chromium, GNOME Text Editor, and kitty normal/command paths are live-proven, and Chromium renderers are sandbox-attested; additional terminals, sandbox packaging formats/applications, and extended-duration soak behavior remain.
...
3. **Release boundary:** the checked Arch candidate, current/old-metadata handoff, private-session direct replacement, real user-systemd restart/restore, guarded package-removal preparation/rollback, and unsupported-future-schema refusal with byte-preserving package transactions are complete. An actual host package-installed upgrade, automatic cross-user old-upgrade invocation, live production multi-user removal, production publication/key operations, and live external-user installation remain unproven; migration rollback becomes a concrete requirement only when schema 2 exists. Detailed evidence belongs in [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md).
