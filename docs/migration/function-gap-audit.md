...
| Frontend menus/configuration | Scene and ASR candidate display/filtering, F7 scene selection, F8 ASR model selection/reload, configured-key scene/ASR paging, and persisted Tap/Hold/Both timing pass with zero unintended commits and exact restoration | Localization breadth remains |
...
| Fcitx frontend | Persistent keys, live Tap/Hold/Both timing, menus, filtering, i18n, notifications, owner recovery, plus live normal, surrounding-command, primary-fallback, toolkit, scene/ASR display/filter, scene/ASR selection/paging, F8 model selection/reload, information/error notifications, focus, and owner-loss outcomes | Localization breadth, cross-provider switching, and broader multi-application proof |
