...
3. **Release boundary:** the Arch `x86_64` package recipe, extracted runtime, and isolated package transactions are deterministic, but rollback, running-service handoff, stale-owner cleanup, signing, repository publication, and live external-user installation remain unproven.
