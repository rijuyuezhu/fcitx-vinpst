...
| Candidate UI | Result, scene, ASR, and paging menus. | Result candidates, scene menu, and installed-model-aware ASR menu support cursor, paging, digit, enter, escape, mouse selection, and legacy slash filtering with UTF-8/Ctrl editing. Richer labels/i18n remain. | Mostly done |
...
6. Fcitx UX parity is incomplete: the searchable scene menu, installed-model-aware ASR menu, six persistent KeyLists, and Tap/Hold/Both state machine are deterministically tested but not live-desktop proven; richer menu labels/i18n and rich notifications remain missing or partial.
