...
| Hotkeys | Fcitx config for normal, command, scene menu, ASR menu, paging, tap/hold/both, debounce. | Normal, command, scene-menu, and ASR-menu triggers are persistent legacy-named Fcitx KeyLists with immediate reload plus temporary environment overrides. Tap/Hold/Both, 80 ms debounce, 300 ms hold threshold, and 500 ms release tail are implemented. Paging/search-key configuration remains. | Mostly done |
...
| Candidate UI | Result, scene, ASR, and paging menus. | Result candidates, a minimal scene menu, and an installed-model-aware ASR menu support cursor, paging, digit, enter, escape, and mouse selection. Legacy search and richer labels/i18n remain. | Partial |
...
| Config | JSON core config plus Fcitx addon config and CLI mutation. | Typed JSON validation, pointer get/set/edit, resource-specific mutations, persistent frontend KeyLists, and TriggerMode exist. Paging/search keys and other legacy frontend fields remain unapplied but are preserved on write. | Mostly done |
...
6. Fcitx UX parity is incomplete: the minimal scene menu, installed-model-aware ASR menu, persistent KeyLists, and Tap/Hold/Both state machine are deterministically tested but not live-desktop proven; paging/search configuration, legacy search, richer menu labels/i18n, and rich notifications remain missing or partial.
