...
3. **GUI result-path breadth:** common configuration/interaction, checksum-verified model install with rendered reconciliation and inactive managed removal, and command ASR provider published-script config-only recovery are proven. Text-adapter recovery, required-environment confirmation, managed script update/replacement, and remaining resource mutation outcomes still need representative real-session evidence.
