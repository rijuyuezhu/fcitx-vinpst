...
| Distro packaging and upgrades | Partial: the checked Arch `x86_64` package, isolated transaction/signature tests, signed release-gate inventory, candidate promotion, owner diagnostics, explicit systemd-user handoff, and executable-replacement restart for daemons started by the new systemd unit are deterministic. Production publication, first-upgrade/direct-activation/removal handoff, incompatible-state rollback, production key operations, external-user regression, and live installed-desktop proof remain. See [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md). |
...
| User install | Temporary-HOME profiles, direct per-user activation, staged systemd-backed activation, environment wrapper, native runtime bundle, checked Arch package construction, and deterministic subsequent-upgrade restart for the new systemd unit | First-upgrade/direct-activation/removal handoff, incompatible-state rollback, version-selection, production repository, and live external-user policy |
...
3. **Release boundary:** the checked Arch candidate and subsequent executable-replacement restart for daemons started by the new systemd unit are deterministic, but production publication, first-upgrade/direct-activation/removal handoff, incompatible-state rollback, production key operations, destructive stale-owner policy, and live external-user installation remain unproven. Detailed evidence belongs in [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md).
