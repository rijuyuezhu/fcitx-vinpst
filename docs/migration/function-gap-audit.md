...
The core **real desktop native-dictation alpha** path is live-proven through Fcitx trigger -> a preflight-verified isolated PipeWire virtual source -> native ASR -> partial/input-panel updates -> commit. The same installed profile also proves GTK3/Qt6/Chromium application commits, local adapter-backed surrounding-text replacement, zero-delete Wayland primary-selection fallback with exact selection restoration, scene/ASR menu display/filtering, scene selection, configured-key scene paging with exact profile restoration, F8/Enter model selection from streaming Zipformer to offline Paraformer followed by a real commit and restoration to streaming recognition, information and daemon-originated error notifications, two-context focus handoff, verified daemon-owner loss, and same-provider reload followed by another recognition. No physical speaker or microphone is used by the retained injected-audio evidence. The active target is now physical microphone/device breadth, live ASR paging, cross-provider switching, localization breadth, broader cross-application behavior, and one external provider.
...
| Frontend menus/configuration | Scene and ASR candidate display/filtering are live-proven; F7 scene selection, F8 ASR model selection/reload, and configured-key scene paging pass with zero unintended commits and exact restoration | Live multi-page ASR paging and localization breadth remain |
...
| Fcitx frontend | Persistent keys, Tap/Hold/Both, menus, filtering, i18n, notifications, owner recovery, plus live normal, surrounding-command, primary-fallback, toolkit, scene/ASR display/filter, scene selection/paging, F8 model selection/reload, information/error notifications, focus, and owner-loss outcomes | Live multi-page ASR paging, localization breadth, cross-provider switching, and broader multi-application proof |
