...
| Command and remote ASR | Batch/streaming command protocols, raw-PCM-to-WAV bridging, independent Whisper, OpenAI-compatible multipart WAV/Bearer/model/language/prompt transport, enforced HTTP deadlines, final-only remote recognition, prepare-failure preservation, real F8 success/failure roundtrips, and deterministic proxy/`NO_PROXY`, 429/503, timeout, self-signed TLS rejection, DNS failure, connection-refusal and secret-redaction process coverage | Real hosted-provider behavior, authenticated/corporate proxies and custom CAs, provider-specific outage/rate-limit policy, and credential rotation/custody |
