...
3. Prepare-before-swap warm reload and busy-time deferral are implemented; idle reload preparation is still synchronous rather than an observable background worker.
...
The next target is **real desktop native-dictation alpha**. First prove the complete native path, including streaming partial preedit, in a real Fcitx session. Then add the non-blocking reload worker and port the remaining live-registry model families. Frontend menus/configuration and packaging should advance in parallel where they directly support that path.
