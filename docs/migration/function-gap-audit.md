...
| Fedora 43 / openSUSE Leap 16.0 RPM | distro-specific release builds plus isolated install/upgrade/remove transaction baseline; tag jobs consume the checked source archive and publish release 1 only | Hosted DNF/Zypper repositories, RPM signing, SELinux review, and live desktop scriptlet evidence. |
| Nix | locked closure build baseline plus release-gated public Cachix publication from the checked source archive | Pin the public substituter key after cache creation; additional architectures and long-term retention policy remain. |
...
4. **Operational breadth after 0.1.0:** hosted-provider policies, additional devices/applications, long-duration soak, Flatpak desktop integration, hosted RPM repositories/signing, additional Nix architectures/cache-retention policy, and GUI semantic-tree support remain explicitly deferred and documented.
