...
| Hotkeys | Fcitx config for normal, command, scene menu, ASR menu, paging, tap/hold/both, debounce. | Normal, command, scene-menu, ASR-menu, previous-page, and next-page keys are persistent legacy-named Fcitx KeyLists with immediate reload. Tap/Hold/Both, 80 ms debounce, 300 ms hold threshold, and 500 ms release tail are implemented. Legacy menu search behavior remains. | Mostly done |
...
6. Fcitx UX parity is incomplete: the minimal scene menu, installed-model-aware ASR menu, six persistent KeyLists, and Tap/Hold/Both state machine are deterministically tested but not live-desktop proven; legacy search, richer menu labels/i18n, and rich notifications remain missing or partial.
