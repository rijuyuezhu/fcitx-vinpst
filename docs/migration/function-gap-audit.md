...
| Frontend menus/configuration | Scene and ASR candidate display/filtering, F7 scene selection, F8 ASR model selection/reload, configured-key scene/ASR paging, persisted Tap/Hold/Both timing, and installed-catalog zh_CN Scene/ASR titles/status plus localized scene-information text and information/error summaries pass with zero unintended commits and exact English/original-locale restoration | ASR-switch/configuration localization and additional locales remain |
...
| Fcitx frontend | Persistent keys, live Tap/Hold/Both timing, menus, filtering, installed-catalog zh_CN Scene/ASR and scene-info/error-summary notification localization, notifications, owner recovery, plus live normal, surrounding-command, primary-fallback, toolkit, scene/ASR display/filter, scene/ASR selection/paging, F8 same/cross-provider selection/reload and failure preservation, information/error notifications, focus, and owner-loss outcomes | ASR-switch/configuration localization, additional locales, and broader multi-application proof |
