...
| Hotkeys | Fcitx config for normal, command, scene menu, ASR menu, paging, tap/hold/both, debounce. | Normal, command, and scene-menu triggers have legacy-compatible defaults plus environment overrides; persistent Fcitx config, ASR menu trigger, and tap/hold modes remain. | Partial |
...
| Candidate UI | Result, scene, ASR, and paging menus. | Result candidates plus a minimal scene menu with cursor, paging, digit, enter, escape, and mouse selection. ASR menu and legacy search remain. | Partial |
...
6. Fcitx UX parity is incomplete: the minimal scene menu is implemented and C++/D-Bus tested but not live-desktop proven; ASR menu, frontend config UI, tap/hold/both behavior, legacy scene search, and rich notifications remain missing or partial.
