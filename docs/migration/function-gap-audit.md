...
| Hotkeys | Fcitx config for normal, command, scene menu, ASR menu, paging, tap/hold/both, debounce. | Normal, command, scene-menu, and ASR-menu triggers are persistent legacy-named Fcitx KeyLists with immediate reload plus temporary environment overrides. Paging, tap/hold/both, and debounce configuration remain. | Partial |
...
| Config | JSON core config plus Fcitx addon config and CLI mutation. | Typed JSON validation, pointer get/set/edit, resource-specific mutations, and persistent frontend KeyList config exist. Trigger mode, paging/search keys, and other legacy frontend fields remain unapplied but are preserved on write. | Mostly done |
...
6. Fcitx UX parity is incomplete: the minimal scene menu, installed-model-aware ASR menu, and persistent four-KeyList frontend config are deterministically tested but not live-desktop proven; tap/hold/both behavior, paging/search configuration, legacy search, richer menu labels/i18n, and rich notifications remain missing or partial.
