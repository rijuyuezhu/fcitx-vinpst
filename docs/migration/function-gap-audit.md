...
3. Offline Silero VAD is implemented; complete online endpoint behavior, warmup, decode timeout enforcement, and warm reload semantics remain incomplete.
...
The next target is **real desktop native-dictation alpha**. First prove the complete native path, including streaming partial preedit, in a real Fcitx session. Then finish endpoint/timeout/warm-reload semantics and port the remaining live-registry model families. Frontend menus/configuration and packaging should advance in parallel where they directly support that path.
