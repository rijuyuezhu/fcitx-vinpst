...
| Audio | Typed PCM, processing, mock/file sources, capture-before-session ordering, reusable inactive PipeWire streams, bounded generation-safe idle teardown, target-change rebuild and opt-out, serialized D-Bus recording transactions, structured cold-start and first-buffer diagnostics | Real capture proof |
