...
| M6 Release readiness | partial | The checked Arch package, signed candidate pipeline, current-metadata automatic handoff, guarded old-systemd reload/restart, guarded old-direct termination/reactivation, private-session direct replacement proof, and real user-systemd restart/restore proof are complete; a package-installed upgrade, automatic cross-user old-metadata invocation, removal handoff, incompatible-state rollback, production publication/key operations, live external-user proof, and external-user regression remain. Detailed evidence belongs in [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md). |
...
- validate the current-metadata restart and guarded old-metadata handoff through an actual package-installed upgrade, then define an automatic cross-user trigger, removal handoff, and incompatible-state rollback; keep every handoff conditional, identity-guarded, and post-verified;
