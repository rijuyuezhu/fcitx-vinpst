...
| M6 Release readiness | partial | Shared checked native-runtime selection, the Arch package/repository/signature/candidate pipeline including the Rust GUI, current/old-owner handoff and guarded removal, and unsupported-future-schema preservation are complete. The RPM-family baseline builds two releases, validates payload/rpaths/linkage/scriptlets/GUI, and proves user-namespace chroot install/upgrade/verify/removal. Flatpak host routing, permission diagnostics, and service generation are deterministic. A real Flatpak bundle gate, Debian package, supported Fedora/openSUSE builds and DNF/Zypper/signing/SELinux/live-scriptlet proof, actual host package-installed upgrade, production multi-user lifecycle, publication/key operations, and unrelated-machine regression remain. Detailed evidence belongs in [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md). |
| M7 Rust management GUI | active interactive baseline | Iced Control/Resources/LLM/Hotwords pages, typed config validation, redacted diagnostics, headless package self-check, desktop entry/icons, and Arch integration are complete. Control edits global/audio/VAD plus active provider/scene through shared atomic persistence with backup/conflict/session guards, requests daemon reload, and directly starts/stops normal recording over D-Bus. Resource lifecycle/progress/cancellation, owner subscriptions/reconnect, command-mode selection, localization/accessibility, and real Wayland/X11 interaction proof remain. |
...
Advance three release-critical tracks in parallel: add GUI resource lifecycle/progress and daemon reconnect on the implemented typed mutation baseline; turn the deterministic Flatpak runtime contract into a checked bundle/build/install transaction; and validate the RPM baseline inside supported Fedora/openSUSE environments with repository/signing/SELinux/live-scriptlet coverage while starting Debian packaging. Keep the existing desktop, provider, network, lifecycle, Arch, RPM, shared-config-persistence, and GUI save/recording evidence green, then continue hosted-provider and additional physical-device proof where credentials/hardware are available. Do not regress into a second C++ GUI or duplicate business logic in presentation code.
