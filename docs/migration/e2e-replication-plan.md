...
| M6 Release readiness | partial | The checked Arch package, signed candidate pipeline, explicit handoff, private-session direct-activation replacement/reactivation proof, and real user-systemd replacement/restart/restore proof are complete for current metadata; a package-installed upgrade, production publication, first-upgrade/removal handoff, incompatible-state rollback, production key operations, live external-user proof, and external-user regression remain. Detailed evidence belongs in [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md). |
...
- validate the proven user-systemd restart through an actual package-installed upgrade, then define first-upgrade/removal handoff, incompatible-state rollback, and destructive direct-PID stale-owner behavior; keep the explicit systemd-user handoff conditional and post-verified;
