...
- Keep the selected source, bundled Linux x86_64 tarball, Arch x86_64, Debian 12 amd64, Ubuntu 24.04 amd64, Fedora 43 RPM, openSUSE Leap 16.0 RPM, Flatpak x86_64, and Nix/Cachix release-channel matrix green.
- Keep the completed one-source boundary green: Arch, Debian, both RPM jobs, Flatpak, and release Nix consume the exact archive generated by the source job; the bundled tarball derives from the tested Ubuntu payload; Arch, both RPM jobs, and Flatpak recheck the consumed archive digest before publication selection.
