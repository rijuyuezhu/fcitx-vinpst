...
Continue the GUI track with broader resource-specific error taxonomy and non-text keyboard/accessibility-tree support, then extend the completed niri/Wayland text-interaction proof to X11 and the remaining portal/opener/notification/install/mutation result paths. Keep the recorded release and platform gaps intact, but defer new packaging work until the GUI management baseline advances; retained package and desktop evidence must remain green.
