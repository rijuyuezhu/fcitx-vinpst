...
| M7 Rust management GUI | active interactive baseline | Control editing/recording and Resources model install/update/discovery/inactive removal use shared typed persistence/registry safety. Progress/cancellation, provider/script lifecycle, owner reconnect, command-mode selection, localization/accessibility, and live GUI proof remain. |
