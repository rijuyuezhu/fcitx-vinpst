...
Continue the GUI track by moving remaining blocking actions onto the plain-thread boundary, then implement non-text keyboard/accessibility-tree support and extend the completed niri/Wayland text-interaction proof to X11 and the remaining portal/opener/notification/install/mutation surfaces. Keep the recorded release and platform gaps intact, but defer new packaging work until the GUI management baseline advances; retained package and desktop evidence must remain green.
