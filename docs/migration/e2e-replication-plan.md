...
| M6 Release readiness | partial | Arch package/repository/signature/candidate, Debian 12/Ubuntu 24.04 Docker transactions, locked Nix build, and RPM-family transaction baseline are complete. Flatpak host routing/permissions/service generation are deterministic. Real Flatpak bundle, Fedora/openSUSE repository/signing/SELinux/live-scriptlet proof, Nix binary-cache publication, actual host upgrade, production multi-user lifecycle, publication keys, and unrelated-machine regression remain. |
| M7 Rust management GUI | active interactive baseline | Control editing/recording and Resources model install/update/discovery/inactive removal use shared typed persistence/registry safety. Serialized non-activating owner polling automatically recovers daemon status after restart. Progress/cancellation, provider/script lifecycle, signal-driven owner subscriptions, command-mode selection, localization/accessibility, and live GUI proof remain. |
...
Advance three release-critical tracks in parallel: add GUI progress/cancellation, provider/script lifecycle, and signal-driven owner reconciliation; complete the checked Flatpak bundle/build/install/update/remove transaction; and validate RPM inside Fedora/openSUSE containers with repository/signing/SELinux/live-scriptlet coverage. Add Nix binary-cache publication only after key/retention policy is defined, and keep Arch/Debian/Nix/RPM plus desktop evidence green.
