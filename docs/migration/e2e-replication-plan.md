...
6. Keep the live-proven focus-handoff, owner-loss, same-provider reload, default physical-microphone dictation, scene/ASR selection and paging, installed-catalog zh_CN menu localization, F8 model/cross-provider/independent-Whisper selection, Tap/Hold/Both timing, and information/error notification paths green; next exercise additional device switching, remote hosted ASR/network failure behavior, and remaining localization surfaces/locales.
