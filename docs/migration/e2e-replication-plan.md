...
- validate current systemd restart and direct next-request reactivation in a live installed session, then define first-upgrade/removal handoff, incompatible-state rollback, and destructive direct-PID stale-owner behavior; keep the explicit systemd-user handoff conditional and post-verified;
