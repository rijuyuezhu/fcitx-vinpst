...
7. Keep both live-proven command paths and the deterministic `vinput llm test` network-semantics gate green; next prove real hosted-provider credential operations, authenticated/corporate proxy or custom-CA behavior, provider-specific outage policy, and cross-application recovery.
