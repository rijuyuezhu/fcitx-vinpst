...
| M6 Release readiness | partial | Checked release-time native-runtime bundle selection, the Arch package and signed candidate pipeline, current-metadata automatic handoff, guarded old-systemd/direct handoff, private-session direct replacement proof, real user-systemd restart/restore proof, guarded cross-user removal dispatch/rollback, and unsupported-future-schema refusal with byte-preserving install/upgrade/pkgrel rollback/removal are complete. The external-user Arch lifecycle guide and isolated command smoke are complete. An actual host package-installed upgrade, automatic cross-user old-metadata upgrade invocation, live production multi-user uninstall, production publication/key operations, and regression on an unrelated external machine remain. A real schema-migration rollback gate becomes applicable only when schema 2 exists. Detailed evidence belongs in [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md). |
