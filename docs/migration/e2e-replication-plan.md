...
Advance three release-critical tracks in parallel: add GUI progress/cancellation, provider/script lifecycle, and signal-driven owner reconciliation; prove the checked Flatpak extension inside a real desktop/Fcitx/PipeWire/host-systemd session and define its repository/signing policy; and validate RPM inside Fedora/openSUSE containers with repository/signing/SELinux/live-scriptlet coverage. Add Nix binary-cache publication only after key/retention policy is defined, and keep Arch/Debian/Nix/RPM/Flatpak plus desktop evidence green.
