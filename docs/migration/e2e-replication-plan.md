...
7. Keep both live-proven command paths and the deterministic `vinput llm test` network-semantics gate green; next prove real hosted-provider credential/CA lifecycle operations, PAC, NTLM/Kerberos, HTTPS proxy endpoints, production TLS-interception policy, provider-specific outage behavior, and cross-application recovery.
