...
Continue the GUI track: add provider/adapter removal, managed provider script editing, required environment-value entry, and richer resource details/errors; then replace safe owner polling with signal-driven reconciliation and add command-mode, localization/accessibility, and live Wayland/X11 proof. Keep the recorded release and platform gaps intact, but defer new packaging work until the GUI management baseline advances; retained package and desktop evidence must remain green.
