...
| M7 Rust management GUI | active interactive baseline | Control editing/recording and Resources model install/update/discovery/inactive removal use shared typed persistence/registry safety. Model install/update reports typed phase/byte progress, supports cleanup-safe cancellation and exact-selector retry, rejects stale completions, and requests cancellation on GUI shutdown. Resources and LLM now install/update registry command providers and text adapters with managed script publication, user-defined-entry refusal, preserved environment values, atomic validated config persistence, daemon reload, cooperative cancellation, retry, and stale-completion rejection. They also remove only exact managed-root entries, reject active providers and user-defined resources, commit validated config removal first, and clean up the unreferenced script. Serialized non-activating owner polling automatically recovers daemon status after restart. Provider script editing, required environment-value entry, richer details/errors, signal-driven owner subscriptions, command-mode selection, localization/accessibility, and live GUI proof remain. |
...
Continue the GUI track: add managed provider script editing, required environment-value entry, and richer resource details/errors; then replace safe owner polling with signal-driven reconciliation and add command-mode, localization/accessibility, and live Wayland/X11 proof. Keep the recorded release and platform gaps intact, but defer new packaging work until the GUI management baseline advances; retained package and desktop evidence must remain green.
