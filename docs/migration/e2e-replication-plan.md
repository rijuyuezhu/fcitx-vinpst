...
| M6 Release readiness | partial | Arch `x86_64` package construction and isolated pacman install/upgrade/same-version-rollback/uninstall are deterministic; incompatible-state rollback, running-service handoff, repository publication, install docs, live installed proof, and external-user regression remain |
...
- define incompatible-state rollback, running-service upgrade/removal handoff, and stale-owner migration behavior;
