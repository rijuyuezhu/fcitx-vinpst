...
Advance three release-critical tracks in parallel: add GUI resource lifecycle/progress and daemon reconnect on the implemented typed mutation baseline; turn the deterministic Flatpak runtime contract into a checked bundle/build/install transaction; and add the next non-Arch package recipe. Keep the existing desktop, provider, network, lifecycle, Arch, shared-config-persistence, and GUI save/recording evidence green, then continue hosted-provider and additional physical-device proof where credentials/hardware are available. Do not regress into a second C++ GUI or duplicate business logic in presentation code.
