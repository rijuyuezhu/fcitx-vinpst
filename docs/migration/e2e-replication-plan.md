...
1. Implemented and WAV-proven for SenseVoice, Paraformer, Qwen3 ASR, and Moonshine v1: map registry/local `vinput_model` metadata into native sherpa config; unknown and unsupported family names remain explicit.
2. Implemented for online transducer and Zipformer2 CTC; Zipformer2 CTC is WAV-proven. Add Dolphin and other remaining layouts in registry-priority order.
...
2. Port Dolphin and other remaining live-registry model families.
