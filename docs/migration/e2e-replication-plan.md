...
2. Implemented deterministically: the F8 ASR menu exposes target/effective/loading/error state, scans flat and legacy installed-model layouts, and persists provider/model selection before background reload. The normal, command, scene-menu, ASR-menu, previous-page, and next-page keys are persistent legacy-named Fcitx KeyLists; both menus consume the current paging lists, including keypad defaults. TriggerMode implements Tap/Hold/Both with the legacy timing constants; valid environment values remain temporary trigger overrides, and unknown legacy fields survive writes. Add legacy menu search next.
...
3. Prove the scene menu, installed-model ASR menu, persistent trigger/paging keys, and Tap/Hold/Both timing live, then add legacy search/i18n details needed for real desktop validation.
