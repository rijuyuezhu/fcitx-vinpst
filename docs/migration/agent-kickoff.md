...
The active priority is **M7: Rust management GUI**. Broaden resource-specific error taxonomy and close the explicit accessibility blocker: every blocking HTTP/D-Bus/filesystem/process action now uses the named plain-thread task boundary, and text focus, clipboard, Fcitx5/Rime input, bilingual titles, and page shortcuts are live-proven on niri/Wayland, while non-text keyboard activation and an assistive-technology tree remain unavailable in the current Iced 0.14 widget/runtime boundary. Extend live result-path proof to X11 plus the portal, desktop opener, startup notification card, install/recovery panels, and mutation forms. Treat new packaging and unrelated desktop breadth as deferred expansion unless a regression, blocker, or explicit user request requires it.
