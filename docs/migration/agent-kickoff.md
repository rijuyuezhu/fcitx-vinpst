...
- Frontend notifications: local errors and scene/ASR switch confirmations use translated Fcitx notifications with legacy icons/timeouts and stderr fallback. The retained addon subscribes to `DaemonNotification(ssss)` through the Fcitx bus, and current-generation background ASR reload failures are emitted and session-bus tested; broader notification categories and live desktop presentation remain.
