...
The active priority is **M7: Rust management GUI**. Broaden resource-specific error taxonomy, restore the legacy startup notification feed/details/read-state, complete localization/accessibility, and obtain real Wayland/X11 interaction proof. Treat new packaging and desktop breadth as deferred expansion unless a regression, blocker, or explicit user request requires it.
