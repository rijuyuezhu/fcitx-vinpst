...
Normal/command application paths, clipboard fallback, scene/ASR selection and paging, persisted Tap/Hold/Both modes, notifications, focus/owner recovery, reload, and one real model-switch roundtrip are live-proven. The next proof is physical-device and application breadth, localization, cross-provider switching, and one external provider flow.
...
- Persistent keys and Tap/Hold/Both are implemented and live-proven; filtering, i18n, notifications, and daemon recovery are implemented, with notification/recovery paths also live-proven.
...
- Cross-provider switching, localization breadth, physical-device behavior, broader cross-application behavior, and an external provider flow remain unproven.
