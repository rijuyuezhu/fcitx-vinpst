...
The active priority is **M7: Rust management GUI**. Broaden resource-specific error taxonomy, finish scene/form/install localization on top of the completed English/zh_CN global-Control-Hotwords and Resources/LLM resource-projection layers, complete accessibility, and obtain real Wayland/X11 interaction proof for the portal, desktop opener, startup notification card, and core forms. Treat new packaging and desktop breadth as deferred expansion unless a regression, blocker, or explicit user request requires it.
