...
- The Rust/Iced GUI provides Control, Resources, LLM, and Hotwords pages. It uses shared typed config/protocol/registry APIs; supports scene and LLM-provider lifecycle, configured-only scene provider selection, typed custom Local/Command/Remote ASR provider creation and configured-provider editing, typed custom text-adapter creation/config editing and status/start/stop controls, model and managed provider/adapter install/update, progress, cancellation, retry, guarded removal, production-adapter connectivity testing, hotword provider/path/content lifecycle with conflict-aware atomic writes, committed-file preservation after unconfirmed activation, validated activation retry, and non-activating owner recovery; and is installed by the checked package paths.
