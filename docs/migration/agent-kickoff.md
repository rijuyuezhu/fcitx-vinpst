...
Normal/command application paths, default physical-microphone dictation, local-adapter and loopback OpenAI-compatible HTTP-provider command replacement, clipboard fallback, scene/ASR selection and paging, installed-catalog zh_CN menu localization, persisted Tap/Hold/Both modes, notifications, focus/owner recovery, reload, one real model-switch roundtrip, one internal-sherpa-to-external-command-process roundtrip, one independent whisper.cpp recognizer/model roundtrip, and one unavailable-remote-provider failure/recovery roundtrip are live-proven. The next proof is additional device/application breadth, real cloud text-provider behavior, successful remote hosted ASR transport, and remaining localization surfaces/locales.
...
- The ASR cross-provider command-process roundtrip is live-proven in two forms: a compatibility child that reuses the original sherpa/Zipformer model, and an independent whisper.cpp v1.9.1 + multilingual `ggml-base.bin` recognizer/model selected through real F8/Enter events. A third gate selects an unavailable remote target and proves that reload failure preserves Zipformer, emits matching daemon/Fcitx errors, restores exact state, and recognizes again. The external OpenAI-compatible text-provider process is also live-proven, but it is a loopback fixture rather than a third-party cloud service. Successful remote hosted ASR credentials/network behavior, real cloud text credentials/network behavior, localized notification/configuration surfaces, additional locales/devices, and broader cross-application behavior remain unproven.
