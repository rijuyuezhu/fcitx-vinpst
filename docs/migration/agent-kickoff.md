...
- Remote text settings, protocol semantics, browser assets, Axum runtime, normal D-Bus daemon startup/provider-selection/reload ownership, `SIGTERM` shutdown, and redacted endpoint diagnostics are deterministic. A real sandboxed Chromium page now passes through the host's non-loopback LAN address with exact Realtime output and cleanup evidence; another physical-device browser proof remains.
