...
The active priority is **M7: Rust management GUI**. Broaden resource-specific error taxonomy, finish install/progress/recovery localization on top of the completed English/zh_CN global-Control-Hotwords, resource-projection, scene/ASR-form, and LLM/adapter-form layers, complete accessibility, and obtain real Wayland/X11 interaction proof for the portal, desktop opener, startup notification card, and core forms. Treat new packaging and desktop breadth as deferred expansion unless a regression, blocker, or explicit user request requires it.
...
- The Rust/Iced GUI provides Control, Resources, LLM, and Hotwords pages, including typed daemon Start/Stop/Restart controls. It uses shared typed config/protocol/registry APIs; supports scene and LLM-provider lifecycle, configured-only scene provider selection, typed custom Local/Command/Remote ASR provider creation, configured-provider editing, and inactive config-only removal, typed custom text-adapter creation/config editing, status/start/stop controls, daemon-runtime-safe ids, and config-only removal, model and managed provider/adapter install/update, progress, cancellation, retry, guarded removal, production-adapter connectivity testing, shell-free native/Flatpak Open Config and notification Details, a bounded current-upstream startup notification feed with locale fallback and monotonic legacy read state, portal-backed hotword path selection, hotword provider/path/content lifecycle with conflict-aware atomic writes, committed-file preservation after unconfirmed activation, validated activation retry, non-activating owner recovery, and typed English/zh_CN presentation for the window shell, navigation, Control, daemon/recording lifecycle, desktop and notification actions, Hotwords, Resources/LLM page chrome and rows, adapter runtime states, secret-safe resource details, scene lifecycle/forms, custom ASR provider forms, LLM provider forms/connectivity results, and text-adapter forms plus fixed mutation outcomes while retaining stable machine ids, candidate counts, and raw diagnostic values; and is installed by the checked package paths.
