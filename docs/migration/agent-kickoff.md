...
- Native desktop install: `sherpa-native-live` accepts supported typed offline/online models, copies the validated `libsherpa-onnx`/`libonnxruntime` bundle, uses wrapper-based D-Bus activation, and passes a real temporary-HOME online-transducer activation round trip. The native addon smoke now drives the production `ApplyBridgeOutcomeToInputContext` path through a concrete Fcitx test frontend and captures the exact transcript in `commitStringImpl`. The old `sherpa-sense-voice-live` alias remains tested; real application and live microphone proof remain.
...
- Native command fallback: with no configured text adapter, command mode preserves the selected text as the safe default, exposes the recognized command as an ASR candidate, and opens the addon candidate menu. The temporary-HOME native addon smoke selects the ASR candidate through a real `fcitx::CandidateList`, captures `deleteSurroundingTextImpl`, and then captures the replacement in `commitStringImpl`; real multi-application behavior remains.
...
- Live partial signals: generation-scoped D-Bus emission is implemented and session-bus tested before stop, with stop-time deduplication. The retained addon monitor can subscribe before D-Bus activation, follows the daemon's unique owner, and rejects other senders. Temporary-HOME native online-transducer smokes now prove `RecognitionPartial` reaches a concrete Fcitx test `InputContext` before Stop in normal and command modes; real application rendering and PipeWire capture remain unproven.
