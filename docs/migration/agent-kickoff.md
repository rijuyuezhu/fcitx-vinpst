...
- Packaging now covers explicit handoff and executable-replacement handoff for current systemd and direct D-Bus activation metadata. A private-session process gate proves direct-owner exit and next-request reactivation, while `just systemd-upgrade-live` proves replacement-triggered PID restart under the real user systemd manager and exact restoration of the original direct-activation profile. A package-installed upgrade, first-upgrade/removal handling, production publication, incompatible-state rollback, and the deferred Rust GUI remain; see [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md) and [`../architecture/gui-contract.md`](../architecture/gui-contract.md).
