...
- Frontend config: normal, command, scene-menu, ASR-menu, previous-page, and next-page keys are persistent legacy-named Fcitx KeyLists with immediate reload; both menus consume the configured paging lists, including keypad defaults. TriggerMode implements Tap/Hold/Both with legacy debounce/hold/release-tail timing, temporary trigger overrides remain, and unknown legacy fields are preserved. Legacy menu search remains.
- Biggest blockers: real Fcitx -> PipeWire -> native ASR -> partial/preedit -> commit proof, remaining sherpa families, legacy frontend search/i18n details, packaging, and remote services.
...
3. Prove the scene menu, installed-model ASR menu, persistent trigger/paging keys, and Tap/Hold/Both timing live, then add legacy search/i18n details.
