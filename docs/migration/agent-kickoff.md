...
- The ASR cross-provider command-process roundtrip is live-proven in two forms: a compatibility child that reuses the original sherpa/Zipformer model, and an independent whisper.cpp v1.9.1 + multilingual `ggml-base.bin` recognizer/model selected through real F8/filter/Enter events. The external OpenAI-compatible text-provider process is also live-proven, but it is a loopback fixture rather than a third-party cloud service. Remote hosted ASR credentials/network behavior, real cloud text credentials/network behavior, localized notification/configuration surfaces, additional locales/devices, cross-provider failure recovery, and broader cross-application behavior remain unproven.
