...
- Native command fallback: with no configured text adapter, command mode preserves the selected text as the safe default, exposes the recognized command as an ASR candidate, and opens the addon candidate menu. The temporary-HOME native addon smoke proves this path; real InputContext replacement remains.
