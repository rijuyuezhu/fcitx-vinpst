...
- Frontend notifications/recovery: local errors and scene/ASR switch confirmations use translated Fcitx notifications with legacy icons/timeouts and stderr fallback. The retained addon subscribes to daemon signals and uses race-free service-owner tracking; owner loss during recording aborts the frontend session with a localized error. Current-generation background ASR reload failures are emitted and session-bus tested; broader notification categories and live desktop presentation remain.
