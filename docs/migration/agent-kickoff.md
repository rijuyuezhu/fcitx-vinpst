...
- Source, bundled Linux x86_64 tarball, Arch x86_64, Debian 12 amd64, Ubuntu 24.04 amd64, Fedora 43 RPM, openSUSE Leap 16.0 RPM, and Flatpak x86_64 now form the selected GitHub release set; Nix publishes through a required public Cachix channel. Native/Flatpak jobs consume one checked source archive (the tarball reuses the tested Ubuntu payload), release assets retain manifest/checksum plus GitHub/Sigstore provenance, notes are generated with git-cliff, and publication remains draft-first with exact remote inventory checks. The remaining work is final candidate rehearsal, artifact-installed desktop validation, cache-secret/key setup, tag publication, and post-publication smoke.
