...
The active priority is **M7: Rust management GUI**. Complete provider script editing, required environment-value entry, richer resource details and error recovery, signal-driven daemon reconciliation, command-mode integration, localization/accessibility, and real Wayland/X11 interaction proof. Treat new packaging and desktop breadth as deferred expansion unless a regression, blocker, or explicit user request requires it.
...
- `main` through `9d31f70` passed the complete CI matrix: Rust, Nix, Debian 12, and Ubuntu 24.04.
