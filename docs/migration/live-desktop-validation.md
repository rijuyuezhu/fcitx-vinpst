...
The deterministic removal handoff gate is now explicit CLI evidence only. `scripts/tests/daemon/run-daemon-removal-handoff-smoke.sh` uses isolated session buses to prove the guarded `vinpst daemon prepare-remove` command across no-owner, idle systemd/direct, and busy-owner cases. Package-manager removal does not dispatch that command or mutate live desktop sessions.
