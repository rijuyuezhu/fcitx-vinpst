...
- cross-provider failure preservation: `just ime-fcitx-cross-provider-failure-live` exposed one `remote-failure-fixture [Remote]` candidate, selected it with real F8/Enter events, observed the target change while Zipformer stayed effective, matched one `asr_backend_reload_failed` signal and one 5000 ms Fcitx `dialog-error` notification to the current daemon/Fcitx PIDs, restored profile and `.bak`, and then produced seven streaming partials plus a final commit under `target/tmp/ime-fcitx-cross-provider-failure-live`; this proves the failure boundary, not remote transport implementation;
- external text provider: `just ime-fcitx-external-text-provider-live` restarted configured text processing against an independent loopback OpenAI-compatible HTTP server, sent one authenticated `/v1/chat/completions` request containing real surrounding selected text and raw ASR text, exposed three Fcitx candidates, selected the HTTP candidate, deleted the original text, committed the exact provider result, and restored profile/`.bak`, service, addon config, local adapter, ASR backend, and daemon ownership under `target/tmp/ime-fcitx-external-text-provider-live`; the trace records only `Bearer` as the auth scheme and never stores the token, and the fixture is not third-party cloud-service proof;
