...
hotword get/set/clear/edit; scene list/ls/add/edit/use/remove
device list/use
...
- live registry install/use/remove exists for model flow, provider list/use exists, but remaining provider UX polish plus LLM commands are still missing; device list/use exists;
...
- `vinput scene list/ls/add/edit/use/remove` inspects and selects configured recognition scenes. **Done for dry-run/output/in-place writes, backup/validation guards, JSON/text output, and README/just smoke.**
