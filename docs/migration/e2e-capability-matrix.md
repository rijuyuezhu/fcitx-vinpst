...
| Select and reload a model/provider | live-proven within `sherpa-onnx` and across the internal/command-provider boundary | real F8/Enter selection switches streaming Zipformer to offline Paraformer, and separately switches to `external-command/external-one-shot`; the external legacy-command process converts raw PCM to WAV, launches a child one-shot daemon, commits final-only text, cleans the WAV, then restores Zipformer with streaming partials; service/profile/backup/Fcitx/backend state is restored exactly | Independent third-party ASR recognizer/model and cross-provider failure/recovery combinations |
...
| Scene and ASR menus | selection/paging, cross-provider selection, and zh_CN menu localization live-proven | real Fcitx clients prove F7/F8 display/filter/Escape, F7 Enter scene selection, F8 Enter same-provider model and external command-provider selection with completed reload, configured-key scene paging, a 14-target ASR menu across `1/2 -> 2/2 -> 1/2`, and installed-catalog `场景 /过滤` / `模型 /过滤` plus `当前：` status text; all gates reject unintended commits and restore profile/service/Fcitx/backend state exactly | Localized notification/configuration surfaces, additional locales, and cross-provider failure/recovery proof |
...
| Provider-backed text processing | live-proven for the configured local command adapter | `sherpa-native-command-live`, runtime adapter identity, virtual-source command ASR, selected-text deletion, and an `adapter-backed:` direct commit; OpenAI-compatible transport remains deterministic | One external provider flow |
...
The installed `sherpa-native-command-live` profile now has retained normal/command evidence for GTK3, Qt6, and Chromium/Ozone, plus real Fcitx-client evidence for default physical-microphone dictation, surrounding-text replacement, Wayland primary-selection fallback, scene selection, configured-key scene and ASR paging, installed-catalog zh_CN Scene/ASR titles/status with English restoration, F8 same-provider model and external command-provider selection/reload, persisted Tap/Hold/Both timing, and information/error notifications. The cross-provider gate proves an external child-process boundary and exact restoration, while explicitly reusing the original sherpa/Zipformer recognizer rather than claiming an independent third-party model. The physical gate uses no playback injection; fallback/localization/trigger/menu/model/provider gates restore the original addon config, scene, profile, activation service, Fcitx process, and effective backend. Remaining behavior includes broader cross-application selected-text behavior, a genuinely independent third-party ASR recognizer/model, localized notification/configuration surfaces and additional locales, additional physical-device switching, cross-provider failure recovery, and an external text-processing provider.
...
1. Validate one external text-provider-backed command transformation and one genuinely independent third-party ASR recognizer/model.
