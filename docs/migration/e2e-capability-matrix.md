...
| Sherpa offline | Multiple families through C API metadata. | Feature-gated official Rust binding; SenseVoice, Paraformer, Qwen3 ASR, and Moonshine v1 pass real registry-model WAV smokes. | Mostly implemented; Dolphin and other absent registry families remain. |
...
| Model metadata | Legacy reads registry/local `vinput_model` metadata and maps family-specific files. | Rust classifies current and legacy registry families; maps SenseVoice, Paraformer, Qwen3 ASR, Moonshine v1, transducer, and Zipformer2 CTC assets/config; validates required files; and preserves unknown future family names. | Partial; Dolphin and other families still need runtime mapping. |
...
- Remaining acceptance: add native layouts for Dolphin and other live-registry families.
...
2. Port Dolphin and other remaining metadata/runtime layouts in registry-priority order.
