...
| Audio capture | partial | capture-first ordering, failure rollback, reusable inactive PipeWire streams, bounded idle-grace teardown, generation-safe expiry, target-change rebuild, and legacy opt-out are deterministic; stop/start race hardening and live capture remain |
