...
| J2: normal dictation with local model | Fcitx trigger starts PipeWire capture, ASR, optional postprocess, commit. | Native offline/online registry families recognize bundled WAV files; `sherpa-native-live` copies the validated runtime bundle and generates wrapper-based activation. A temporary-HOME online transducer is now D-Bus auto-activated by the first status query, reports the installed daemon owner, and returns the exact transcript through `StartRecording`/`StopRecording`. The live Fcitx/PipeWire/native model path remains unproven. | Major real-desktop trigger/partial/commit proof. | Real desktop checklist passes: trigger, preedit, capture, inference, commit into app, `doctor` green. |
