...
| Scene, ASR, and notification localization | selection/paging, cross-provider selection/failure preservation, zh_CN menu localization, and zh_CN scene-info/ASR-switch/error-summary notifications live-proven | real Fcitx clients prove F7/F8 display/filter/Escape, F7 Enter scene selection, F8 Enter same-provider and external-provider selection, unavailable-remote reload failure with old-backend preservation, configured-key scene paging, a 14-target ASR menu across `1/2 -> 2/2 -> 1/2`, installed-catalog `场景 /过滤` / `模型 /过滤` plus `当前：` status text, `语音输入` summaries, `已切换场景到“Command”。`, `已请求切换语音识别到“remote-failure-fixture”。`, verbatim daemon error bodies, old-backend preservation plus nine recovered partials, and English/original-locale restoration; all gates reject unintended commits and restore profile/service/Fcitx/backend state exactly | configuration-surface localization and additional locales |
...
3. Broaden localization to configuration surfaces and additional locales.
