...
| Daemon lifecycle | implemented | direct per-user activation, systemd-backed activation, default user-config discovery with persistent D-Bus updates, status, reload, stop/restart/log plans, owner diagnostics, private-session proof that direct activation resumes with a new PID, and real user-systemd proof that executable replacement increments `NRestarts`, changes `MainPID`, and restores the original direct owner | First upgrade from older activation metadata, an actual package-installed upgrade, and removal hardening |
...
| User installation | deterministic | temporary-HOME activation/runtime recognition plus the checked Arch package, repository, signature, candidate-promotion, explicit handoff gates, a private-session direct-activation replacement/reactivation process gate, and a real user-systemd replacement/restart/restore gate; see [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md) | Actual package-installed upgrade, first upgrade from older activation metadata, removal handoff, production repository/key operations, incompatible-state rollback, and external-user regression |
...
- first upgrade from older systemd or direct D-Bus activation metadata, removal handoff, incompatible-state rollback, and destructive direct-PID stale-owner policy (subsequent atomic replacement is process-proven through immediate user-systemd restart or direct reactivation on the next D-Bus request; explicit conditional handoff remains available);
