...
hotword get/set/clear/edit; scene list/ls/use
device list/use
...
- live registry install/use/remove exists for model flow, provider list/use exists, but remaining provider UX polish plus scene add/edit/remove and LLM commands are still missing; device list/use exists;
...
- `vinput scene list/ls/use` inspects and selects configured recognition scenes. **Done for dry-run/output/in-place writes, backup/validation guards, JSON/text output, and README/just smoke.**
