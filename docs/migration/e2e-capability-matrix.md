...
- Rust implementation reviewed on the current branch through the checked Arch release pipeline and RPM-family build/transaction baseline.
...
| User installation | deterministic | temporary-HOME activation/runtime recognition, the checked external-user Arch lifecycle guide and isolated command smoke, shared checked native-runtime selection, the Arch package/repository/signature/candidate pipeline including `vinput-gui`, automatic ownership-verified cross-user dispatch, private-session and real user-systemd replacement proof, guarded removal rollback, and unsupported-schema preservation. The RPM-family baseline builds two releases, validates payload/rpaths/linkage/scriptlets/GUI, and proves user-namespace chroot install/upgrade/verify/removal with byte-identical future config | Actual host package-installed upgrade, live production multi-user upgrade/removal, Debian and Flatpak packages, supported Fedora/openSUSE builds plus DNF/Zypper/signing/SELinux/live-scriptlet validation, production repository/key operations, and unrelated-machine regression |
| Standalone GUI | packaged interactive management baseline | Rust/Iced Control, Resources, LLM, and Hotwords pages read validated typed config and redact provider diagnostics. Control edits global/audio/VAD settings plus active provider/scene through shared validation, backup, fsync, atomic replacement, external-change detection, and active-session guards; it requests ASR reload and starts/stops normal recording through typed D-Bus. Unit/Clippy and display-independent `--check --offline` package checks pass | Resource lifecycle actions, download progress/cancellation, owner subscriptions/reconnect, command-mode selection, zh_CN/accessibility, and real Wayland/X11 interaction proof |
...
Current CLI gaps are not command-group gaps. Flatpak host-command routing, permission diagnostics, and native/Flatpak user-service installation are deterministic; the remaining Flatpak boundary is an actual bundle/build/install gate. Other gaps are output polish, non-systemd behavior, and further feature-driven extraction from the CLI composition files.
...
- externally hosted repository publication, production signing-key custody/rotation/revocation and independent public-key/fingerprint distribution, Debian/Flatpak packages, and supported-distro RPM repository/signing/SELinux validation;
...
- remaining Rust `vinput-gui` lifecycle, reconnect, localization/accessibility, and live interaction proof; see [`../architecture/gui-contract.md`](../architecture/gui-contract.md).
...
5. Advance Flatpak/Debian packaging, supported-distro RPM repository/live-scriptlet proof, remote live proof, and the remaining Rust GUI lifecycle/live-interaction work without regressing the retained evidence.
