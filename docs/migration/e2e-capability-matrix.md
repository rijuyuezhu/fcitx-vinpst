...
| J2: normal dictation with local model | Fcitx trigger starts PipeWire capture, ASR, optional postprocess, commit. | Native offline/online registry families recognize bundled WAV files; `sherpa-native-live` copies the validated runtime bundle and generates wrapper-based activation. A temporary-HOME online transducer is D-Bus auto-activated and returns the exact transcript. The retained C++ `FrontendBridge` is also proven as the first activation client and returns a typed Commit outcome with the exact raw candidate. The live Fcitx `InputContext`/PipeWire path remains unproven. | Major real-desktop trigger/partial/commit proof. | Real desktop checklist passes: trigger, preedit, capture, inference, commit into app, `doctor` green. |
