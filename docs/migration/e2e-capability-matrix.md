...
| J2: normal dictation with local model | Fcitx trigger starts PipeWire capture, ASR, optional postprocess, commit. | Native offline/online registry families recognize bundled WAV files; temporary-HOME activation now runs `FcitxVinputAddon::ApplyTriggerAction` through the production InputContext sink and a concrete Fcitx test frontend, capturing the exact native transcript in `commitStringImpl`. The remaining unproven segment is a real desktop application plus live PipeWire capture. | Major real-desktop trigger/partial/commit proof. | Real desktop checklist passes: trigger, preedit, capture, inference, commit into app, `doctor` green. |
...
| Sherpa streaming | Implemented. | Native transducer and Zipformer2 CTC mappings exist; legacy endpoint defaults/overrides are forwarded; recognizers run a 200 ms warmup; recorder callbacks stream 800-frame batches, decode hypotheses, and emit deduplicated `RecognitionPartial` signals before stop. The retained addon consumes `StatusChanged` and `RecognitionPartial` through the Fcitx bus and maps partial-first state to active-context preedit. Both online transducer and Zipformer2 CTC pass real registry-model WAV smokes with exact transcript assertions. Native timeout configuration is explicitly diagnostic-only because official decode is synchronous. | Mostly implemented; real desktop proof and reload parity remain. |
