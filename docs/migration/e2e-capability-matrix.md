...
| Select and reload a model/provider | live-proven within `sherpa-onnx`, across command/Whisper/remote boundaries, and for remote prepare failure | real F8/Enter selection switches streaming Zipformer to offline Paraformer, compatibility command and independent Whisper providers, and `remote-http/fixture-remote-asr`; the remote success gate proves multipart WAV/Bearer/model/language/prompt transport plus a final-only application commit, the invalid-scheme gate preserves Zipformer, and deterministic one-shot daemon cases prove plain-HTTP proxy routing, Basic authentication for HTTP and CONNECT, `NO_PROXY`, additional PEM roots through `SSL_CERT_FILE`, retained built-in `WebPKI` verification, 429/503, distinct request and response-body timeouts, untrusted self-signed TLS rejection, DNS failure, connection refusal, and secret redaction; every live gate restores service/profile/backup/Fcitx/backend state exactly | Real hosted-provider operations, PAC, NTLM/Kerberos, HTTPS proxy endpoints, production TLS-interception policy, provider-specific outage/rate-limit behavior, and credential/CA lifecycle operations |
...
| Provider-backed text processing | live-proven for both the local command adapter and an independent loopback OpenAI-compatible HTTP process, with deterministic production-client network semantics | the HTTP gate proves 404 preservation, Bearer/JSON request shape, selected/raw ASR transport, exact candidate replacement, no-selection rejection and restoration; `vinput llm test` additionally proves plain-HTTP proxy routing, Basic authentication for HTTP and CONNECT, `NO_PROXY`, additional PEM roots through `SSL_CERT_FILE`, retained built-in `WebPKI` verification, 429/503 bodies, distinct request and response-body timeout diagnostics, untrusted self-signed TLS rejection, DNS failure, connection refusal, and credential redaction | Real hosted-provider credentials, PAC, NTLM/Kerberos, HTTPS proxy endpoints, production TLS-interception policy, provider-specific outage/rate-limit behavior, credential/CA lifecycle operations, and cross-application cloud recovery |
...
1. Validate the implemented remote ASR client against a real hosted service, including third-party DNS/TLS, PAC, NTLM/Kerberos, HTTPS proxy endpoints, production TLS-interception policy, provider-specific rate-limit/outage behavior, and credential/CA rotation or custody; validate equivalent real cloud text-provider operational behavior.
