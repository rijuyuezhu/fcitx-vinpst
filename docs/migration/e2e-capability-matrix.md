...
| J7: daemon control | Legacy `vinput daemon status/start/stop/restart/log` integrates D-Bus/systemd/logs. | Rust has `daemon status/start/reload-asr`, real user-service `stop/restart/log` execution, dry-run plans, activation service generation, D-Bus owner probe diagnostics, and expanded runtime-status diagnostics for ASR/runtime/text-adapter state. | Mostly implemented; owner/stale-process reporting and fallback UX still need hardening. | User can start/stop/restart/status/log daemon from CLI, including bounded daemon log output, using activation/systemd/user-mode strategy. |
