...
1. Validate the implemented remote ASR client against a real hosted service, including third-party DNS/TLS, authenticated/corporate proxy or custom-CA behavior, provider-specific rate-limit/outage policy, and credential rotation/custody; validate equivalent real cloud text-provider operational behavior.
