...
- legacy has JSON-pointer `config get/set/edit`; Rust now has guarded JSON-pointer `get/set`, `config get --exists` for missing-pointer scripting, JSON parsing plus `--string`, and validated editor-based `edit`;
...
- `vinput config get <json-pointer>` and `set <json-pointer> <value>` work with type-aware parsing, `get --exists`, `--string` literal values, and validation. **Done for existing JSON pointers, missing-pointer existence checks, dry-run, output writes, in-place backup, and validation guards.**
