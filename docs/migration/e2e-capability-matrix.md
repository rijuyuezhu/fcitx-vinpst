...
| User installation | deterministic | temporary-HOME activation/runtime recognition, the checked external-user Arch lifecycle guide and isolated command smoke, checked release-time native-runtime bundle selection, the Arch package/repository/signature/candidate pipeline including `vinput-gui`, its desktop entry and icons, automatic ownership-verified cross-user dispatch, private-session direct replacement/reactivation, real user-systemd replacement/restart/restore, guarded removal rollback, and unsupported-schema refusal with byte-identical user config | Actual host package-installed upgrade, live production multi-user upgrade/removal, non-Arch package formats, production repository/key operations, and regression on an unrelated external machine |
...
5. Advance upgrade/repository policy, additional package formats, remote live proof, and the remaining Rust GUI lifecycle/live-interaction work without regressing the retained evidence.
