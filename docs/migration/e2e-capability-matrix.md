...
| Daemon lifecycle | implemented | direct per-user activation, systemd-backed activation, default user-config discovery with persistent D-Bus updates, status, reload, stop/restart/log plans, owner diagnostics, and deterministic executable-replacement exit/restart for daemons started by the new systemd unit | First upgrade from an older unit, direct-activation upgrade handoff, and removal hardening |
...
| User installation | deterministic | temporary-HOME activation/runtime recognition plus the checked Arch package, repository, signature, candidate-promotion, explicit handoff gates, and systemd-backed executable-replacement restart; see [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md) | Real profile, first upgrade from an older unit, direct-activation/removal handoff, production repository/key operations, incompatible-state rollback, and external-user regression |
...
- first upgrade from an older systemd unit, direct-activation and removal handoff, incompatible-state rollback, and destructive direct-PID stale-owner policy (subsequent atomic upgrades of daemons started by the new systemd unit restart automatically; explicit conditional handoff remains available);
