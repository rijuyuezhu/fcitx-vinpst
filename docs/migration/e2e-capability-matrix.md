...
| Provider-backed text processing | live-proven for both the local command adapter and an independent loopback OpenAI-compatible HTTP process, with deterministic production-client network semantics | the HTTP gate proves 404 preservation, Bearer/JSON request shape, selected/raw ASR transport, exact candidate replacement, no-selection rejection and restoration; `vinput llm test` additionally proves plain-HTTP proxy routing, proxy-URL Basic authentication, `NO_PROXY`, 429/503 bodies, distinct request and response-body timeout diagnostics, self-signed TLS rejection, DNS failure, connection refusal, and credential redaction | Real hosted-provider credentials, authenticated/corporate proxies/custom CAs, provider-specific outage/rate-limit policy, and cross-application cloud recovery |
...
1. Validate the implemented remote ASR client against a real hosted service, including third-party DNS/TLS, HTTPS CONNECT or corporate proxy behavior, custom-CA behavior, provider-specific rate-limit/outage policy, and credential rotation/custody; validate equivalent real cloud text-provider operational behavior.
