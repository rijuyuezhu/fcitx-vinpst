...
| Standalone GUI | packaged management baseline | Rust/Iced Control, Resources, LLM, and Hotwords pages read validated typed config, redact provider diagnostics, query daemon status directly over D-Bus, and pass unit/Clippy plus display-independent `--check --offline` package checks | Editable forms and lifecycle actions, progress/cancellation, owner subscriptions/reconnect, zh_CN/accessibility, and real Wayland/X11 interaction proof |
