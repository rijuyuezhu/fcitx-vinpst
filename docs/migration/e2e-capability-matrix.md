...
Current CLI gaps are not command-group gaps. Flatpak CLI host-command routing is deterministic and preserves logical tool/wrapper diagnostics, but Flatpak-specific service-unit rewriting and missing-permission diagnostics remain. Other gaps are output polish, non-systemd behavior, and further feature-driven extraction from the CLI composition files.
