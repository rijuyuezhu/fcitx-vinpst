...
- Native sherpa evidence: Registry model `model.sherpa-onnx.streaming-zipformer-en-20m-2023-02-17` was SHA-256 verified, completed the 200 ms warmup, and recognized bundled `test_wavs/0.wav` as `THE YELLOW LAMPS WOULD LIGHT UP HERE AND THERE THE SQUALID QUARTER OF THE BRAFFLEL` through `just sherpa-online-transducer-local-smoke`. Registry model `model.sherpa-onnx.zipformer-multi-zh-hans` was SHA-256 verified and recognized bundled `test_wavs/0.wav` as `对我做了介绍那么我想说的是大家如果对我的研究感兴趣` through `just sherpa-offline-transducer-local-smoke`. Registry model `model.sherpa-onnx.dolphin-base-ctc-multi-lang-int8` was SHA-256 verified and recognized bundled `test_wavs/0.wav` as `对我做了介绍哈那么我想说的是呢大家如果对我的研究感兴趣呢。` through `just sherpa-dolphin-local-smoke`. Registry model `model.sherpa-onnx.sense-voice-zh-en-ja-ko-yue-int8` was downloaded from live `xifan2333/vinput-registry`, verified by SHA-256, extracted, and recognized bundled `test_wavs/zh.wav` as `开放时间早上九点至下午五点` through `just sherpa-sense-voice-local-smoke`. Registry model `model.sherpa-onnx.qwen3-asr-0.6b-int8` was likewise verified and recognized bundled `test_wavs/es1.wav` as `Esta prenda es amplia. Recomiendo elegir una talla menor al habitual.` through `just sherpa-qwen3-local-smoke`. Registry model `model.sherpa-onnx.paraformer-zh-small` was SHA-256 verified and recognized bundled `test_wavs/0.wav` as `对我做了介绍啊那么我想说的是呢大家如果对我的研究感兴趣呢嗯` through `just sherpa-paraformer-local-smoke`. The live Moonshine Tiny int8 model also passed `just sherpa-moonshine-dbus-reload-smoke`: the daemon started on mock, reported `reload_in_progress` with mock still effective, re-read the changed config, swapped to native Moonshine, and recognized the expected English WAV through D-Bus.
...
| Real desktop readiness | 55-65% | Generic native install, runtime-bundle activation, concrete Fcitx InputContext commit/replacement, and activation-safe native partial preedit are deterministic-smoke proven. Real application focus/rendering and PipeWire microphone capture still need proof. |
...
| J2: normal dictation with local model | Fcitx trigger starts PipeWire capture, ASR, optional postprocess, commit. | Native offline/online registry families recognize bundled WAV files; temporary-HOME activation runs `FcitxVinputAddon::ApplyTriggerAction` through a concrete Fcitx test frontend, requires native partial preedit before Stop, and captures the exact final transcript in `commitStringImpl`. The remaining unproven segment is a real desktop application plus live PipeWire capture. | Major real-desktop trigger/partial/commit proof. | Real desktop checklist passes: trigger, preedit, capture, inference, commit into app, `doctor` green. |
| J3: command dictation over selected text | Fcitx command trigger captures selected text or fallback, ASR command scene, LLM/text transform, replace selection. | Surrounding-text and clipboard acquisition exist. A native temporary-HOME addon smoke proves selected text reaches command recording; without an adapter, the original selection and native ASR command are exposed as a command-mode candidate menu. Selecting the ASR entry through the real Fcitx candidate object records selection deletion followed by commit. Adapter-backed replacement and real multi-application behavior still need live proof. | Medium. | In two apps, selected text replacement works; fallback path has clear diagnostics when unavailable. |
...
| Sherpa offline | Multiple families through C API metadata. | Feature-gated official Rust binding; Offline transducer, Dolphin, SenseVoice, Paraformer, Qwen3 ASR, and Moonshine v1 pass real registry-model WAV smokes. | Mostly implemented; other absent registry families remain. |
| Sherpa streaming | Implemented. | Native transducer and Zipformer2 CTC mappings exist; legacy endpoint defaults/overrides are forwarded; recognizers run a 200 ms warmup; recorder callbacks stream 800-frame batches, decode hypotheses, and emit deduplicated `RecognitionPartial` signals before stop. Source-backed deterministic audio now delivers during Begin without stop replay. The activation-safe retained addon monitor follows the current unique owner and maps partial-first state to concrete InputContext preedit; temporary-HOME native smokes prove this before Stop. Both online transducer and Zipformer2 CTC pass real registry-model WAV smokes with exact transcript assertions. Native timeout configuration is explicitly diagnostic-only because official decode is synchronous. | Mostly implemented; real PipeWire desktop proof and reload parity remain. |
...
| Model metadata | Legacy reads registry/local `vinput_model` metadata and maps family-specific files. | Rust classifies current and legacy registry families; maps offline/online transducer, Dolphin, SenseVoice, Paraformer, Qwen3 ASR, Moonshine v1, and Zipformer2 CTC assets/config; validates required files; and preserves unknown future family names. | Partial; other families still need runtime mapping. |
...
- **Implemented:** both the generic profile and the legacy `sherpa-sense-voice-live` alias pass isolated install/status smokes; `just user-ime-sherpa-native-activation-smoke` D-Bus auto-activates a real temporary-HOME generic install, reports the installed daemon owner on the first status response, and recognizes the pinned WAV through `StartRecording`/`StopRecording`.
...
- Implemented: online transducer and Zipformer2 CTC typed metadata/runtime mapping; both are registry-model WAV-proven. The online transducer exact transcript is `THE YELLOW LAMPS WOULD LIGHT UP HERE AND THERE THE SQUALID QUARTER OF THE BRAFFLEL`.
...
- Implemented: Paraformer typed metadata/runtime mapping, complete shared offline recognizer metadata forwarding, and a SHA-256-verified registry-model WAV smoke with exact transcript assertion.
- Implemented: Offline transducer typed encoder/decoder/joiner metadata/runtime mapping and a SHA-256-verified registry-model WAV smoke with exact transcript `对我做了介绍那么我想说的是大家如果对我的研究感兴趣`.
- Implemented: Dolphin typed metadata/runtime mapping and a SHA-256-verified registry-model WAV smoke with exact transcript `对我做了介绍哈那么我想说的是呢大家如果对我的研究感兴趣呢。`.
- Remaining acceptance: add native layouts for other live-registry families.

### P1.2 sherpa streaming backend

Acceptance:

- Implemented: `sherpa-streaming` transducer and Zipformer2 CTC support behind `sherpa-onnx-backend`, with exact registry-WAV smoke assertions for both layouts.
...
2. Port other remaining metadata/runtime layouts in registry-priority order.
