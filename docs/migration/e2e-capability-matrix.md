...
| Daemon lifecycle | implemented | direct per-user activation, systemd-backed activation, default user-config discovery with persistent D-Bus updates, status, reload, stop/restart/log plans, owner diagnostics, and deterministic executable-replacement handoff for current activation metadata: immediate systemd restart or direct reactivation on the next D-Bus request | First upgrade from older activation metadata and removal hardening |
...
| User installation | deterministic | temporary-HOME activation/runtime recognition plus the checked Arch package, repository, signature, candidate-promotion, explicit handoff gates, and executable-replacement handoff for current systemd and direct D-Bus activation metadata; see [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md) | Real profile, first upgrade from older activation metadata, removal handoff, production repository/key operations, incompatible-state rollback, and external-user regression |
...
- first upgrade from older systemd or direct D-Bus activation metadata, removal handoff, incompatible-state rollback, and destructive direct-PID stale-owner policy (subsequent atomic upgrades use immediate systemd restart or direct reactivation on the next D-Bus request; explicit conditional handoff remains available);
