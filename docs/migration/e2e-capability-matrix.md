...
| User installation | deterministic | temporary-HOME activation/runtime recognition plus checked release-time native-runtime bundle selection, the Arch package/repository/signature/candidate pipeline, current/old-metadata handoff gates, private-session direct replacement/reactivation, real user-systemd replacement/restart/restore, guarded cross-user removal dispatch/activation rollback, and unsupported-schema refusal with byte-identical user config across install/upgrade/pkgrel rollback/removal; see [`../architecture/packaging-contract.md`](../architecture/packaging-contract.md) | Actual host package-installed upgrade, live production multi-user removal, production repository/key operations, automatic cross-user old-upgrade invocation, and external-user regression; schema-migration rollback waits for schema 2 |
...
- automatic cross-user invocation of the guarded old-systemd/direct upgrade handoff (removal dispatch is private-session process-proven, but not yet exercised as a live production multi-user uninstall); unsupported future schema refusal/preservation is complete, while migration rollback waits for a second production schema;
