...
- Rust implementation reviewed through `d2f2309`, including the packaged GUI resource lifecycle, the checked Arch release pipeline, Debian 12/Ubuntu 24.04 Docker package transactions, locked Nix build, RPM-family and Flatpak transaction baselines, and hardened long-lived adapter process supervision.
- Legacy reference is `/workspace/fcitx5-vinput`.
- `cargo test --workspace --all-targets`, D-Bus integration, retained-addon tests, and the complete deterministic `just ci` gate pass at `d2f2309`; the last pushed complete remote Rust/Nix/Debian/Ubuntu matrix is `9d31f70`.
