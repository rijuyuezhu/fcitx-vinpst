...
Use `scripts/live/niri/run-ime-fcitx-external-text-provider-live.sh` to distinguish configured OpenAI-compatible HTTP behavior from the local command-adapter path. The gate restarts the daemon because `reload-asr` does not rebuild `text_processor`, records only the `Bearer` scheme rather than the fixture token, requires real surrounding-text deletion and exact candidate commit, then restarts again and verifies `native-command-live-adapter` is restored. Its server is loopback-only and deterministic. The shared command supervisor separately enforces effective deadlines in the parent, kills helper process groups on timeout/output failure, cleans descendants after direct-child exit, and drains both output pipes; text and command-ASR process tests cover ignored large stdin, background descendants, 256 KiB stderr, and independent 1 MiB stdout/stderr overflow rejection. Use `scripts/tests/asr/run-openai-compatible-text-network-smoke.sh` for production-client plain-HTTP proxy routing, proxy-URL Basic authentication, `NO_PROXY`, 429/503, fail-closed 3xx handling with an untouched redirect target, distinct request and response-body timeouts, the legacy 4000 ms text-scene default when no timeout is configured, a 1 MiB cap for success and error response bodies, untrusted self-signed TLS rejection, DNS failure, connection-refusal, and credential-redaction regressions. The same gate loads a temporary PEM root from `SSL_CERT_FILE`, completes Basic-authenticated CONNECT tunnels through both plain-HTTP and TLS-protected HTTPS proxy endpoints to verified HTTPS origins, and relays one local CA-signed TLS interception exchange while retaining only TLS versions and byte counts; built-in `WebPKI` roots remain enabled. Use `scripts/tests/asr/run-provider-ca-rotation-smoke.sh` for same-daemon atomic replacement of the CA file at one unchanged path, including mismatch rejection, idle rollback, and recovery with a newly signed origin for both remote ASR and provider-backed command processing. Real hosted credentials, PAC, NTLM/Kerberos, enterprise TLS-interception policy and certificate deployment, provider-specific outage policy, production CA distribution/revocation, provider credential custody/rotation, and cross-application disconnect recovery remain separate work.
