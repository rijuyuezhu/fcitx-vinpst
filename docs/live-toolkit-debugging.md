...
Use `scripts/live/niri/run-ime-fcitx-external-text-provider-live.sh` to distinguish configured OpenAI-compatible HTTP behavior from the local command-adapter path. The gate restarts the daemon because `reload-asr` does not rebuild `text_processor`, records only the `Bearer` scheme rather than the fixture token, requires real surrounding-text deletion and exact candidate commit, then restarts again and verifies `native-command-live-adapter` is restored. Its server is loopback-only and deterministic. Use `scripts/tests/asr/run-openai-compatible-text-network-smoke.sh` for production-client proxy/`NO_PROXY`, 429/503, explicit timeout, self-signed TLS rejection, DNS failure, connection-refusal, and credential-redaction regressions. Real hosted credentials, authenticated/corporate proxies or custom CAs, provider-specific outage policy, and cross-application disconnect recovery remain separate work.
