...
`just user-ime-activation-owner-smoke` is model-free and verifies that the first `vinput daemon status --json` call activates the installed user service and reports that new owner in the same response. `VINPUT_USER_NATIVE_WAV` is an opt-in deterministic install argument for native profiles; it appends `--wav` to the generated activation service without changing the default live-capture path. `just user-ime-sherpa-native-activation-smoke` requires `VINPUT_SHERPA_MODEL`, `VINPUT_SHERPA_WAV`, and `VINPUT_SHERPA_EXPECT_TEXT`, while `just sherpa-online-transducer-user-activation-smoke` pins the already-proven English online model. `just sherpa-online-transducer-user-frontend-smoke` makes the C++ `FrontendBridge` the first D-Bus client, requires `StartNormal`/`Stop` to produce a typed `Commit` outcome containing the exact transcript and raw candidate, and applies that outcome to a sink with the same preedit/clear/commit ordering as the addon. `just sherpa-online-transducer-user-addon-smoke` runs the same fixture through `FcitxVinputAddon::ApplyTriggerAction` and the production `ApplyBridgeOutcomeToInputContext` path using a concrete test frontend subclass of `fcitx::InputContext`; the exact native transcript reaches `commitStringImpl`. `just sherpa-online-transducer-user-command-addon-smoke` proves selected-text command mode with no configured text adapter: the original selection remains the safe default, the native ASR command remains an `asr` candidate, and choosing that candidate invokes `deleteSurroundingTextImpl` before `commitStringImpl`. These recipes install into a temporary HOME, trigger the generated service through D-Bus, assert the installed daemon owner and exact transcript, then remove the temporary profile without mutating the real desktop profile.

The two native addon recipes also run the Fcitx D-Bus event loop before the daemon exists. They require activation-safe owner tracking, consume `RecognitionPartial` from the installed online recognizer, and assert that the concrete test `InputContext` holds a non-status partial preedit before Stop. The source-backed WAV recorder therefore delivers callback chunks at Begin and returns the cached capture at Stop without replay.
...
`just addon-test` includes a frontend config smoke that roundtrips all six persistent KeyLists plus `TriggerMode` through Fcitx INI serialization, including keypad paging defaults, and proves unknown legacy fields survive writes. It also runs a pure TriggerMode smoke covering Tap, Hold, Both, the 80 ms debounce, 300 ms hold threshold, 500 ms release tail, failed-start rollback, and modifier-only release matching. A separate menu-filter smoke covers multi-term matching, UTF-8 Backspace, Ctrl+W deletion, filter titles, printable digits, and configured paging-key exclusion. The i18n smoke runs under `zh_CN.utf8`, loads the real build-tree `fcitx5-vinput.mo` through the Fcitx domain API, and verifies translated menu, candidate, count, page, and formatted notification strings plus English fallback. The notification smoke pins app name, icons, 3000/5000 ms timeouts, empty-body suppression, and exact stderr fallback. A separate `dbus-run-session` C++ smoke sends real `StatusChanged(s)`, `RecognitionPartial(s)`, and `DaemonNotification(ssss)` messages through the Fcitx bus implementation, verifies string decoding, live preedit composition, payload classification, and callback delivery, then releases the daemon name and verifies `ServiceWatcher` reports available followed by unavailable. The addon D-Bus smoke also starts a normal recording through a separate synchronous client and proves the addon normal trigger reconciles `GetStatus`, adopts the session, stops it, commits the result, and returns the daemon to idle without another Start. The daemon session-bus suite includes 13 tests, including current-generation background ASR reload failure emission; `just addon-install-smoke` verifies both `Configurable=True` metadata and the installed Chinese MO catalog.
