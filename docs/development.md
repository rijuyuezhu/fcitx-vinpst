...
VINPUT_LIVE_NATIVE_WAV=/path/to/speech.wav just ime-fcitx-reload-live
scripts/bench-capture-cold-start.sh --follow
```

The live recipes are intentionally excluded from `just ci`. `just ime-fcitx-native-live` uses a real Fcitx client input context and acoustic playback to verify F9 partial/commit plus F10 deletion/replacement; its JSONL evidence is written under `target/tmp/ime-fcitx-native-live` or an explicit `VINPUT_LIVE_NATIVE_OUT_DIR`. `ime-fcitx-native-command-adapter-live` additionally requires `runtime-status` to list `native-command-live-adapter` and rejects any final commit that does not begin with `adapter-backed:`, preventing the raw-ASR fallback from being counted as adapter proof. This local adapter validates configured adapter transport, not an external provider. `ime-fcitx-menu-live` verifies non-mutating scene/ASR candidate display, slash filtering, two-stage Escape close, and zero commit. `ime-fcitx-focus-live` rejects partial or commit leakage after focus moves to a second context. `ime-fcitx-owner-loss-live` terminates only a verified `vinput-daemon`, requires an unavailable error preedit, and rejects partial-result commit. `ime-fcitx-reload-live` requires an idle ready backend, performs a same-provider reload, verifies the owner/provider/model stay stable, and then requires another acoustic partial/commit result; it does not prove model or provider switching. These Fcitx-client paths passed in an installed `sherpa-native-command-live` profile on 2026-07-30. `ime-gtk3-native-live` and `ime-qt6-native-live` open real toolkit text fields and still require actual desktop F9/F10 events; they deliberately do not synthesize GDK or Qt key events under Wayland. Preserve JSONL output for every claimed result. `just pipewire-check` exercises CLI/daemon audio-device diagnostics without requiring a live PipeWire daemon. For cold-start measurements, enable `RUST_LOG=vinput_audio=debug,vinput_daemon=debug` in the user service, use waits of at least 10 seconds for cold trials and gaps below 2 seconds for warm trials, then run `scripts/bench-capture-cold-start.sh` or pass `--input` to analyze saved journal output. `just capture-cold-start-smoke` validates the parser with a deterministic fixture and is included in `just ci`. Live failures must record whether setup failed at the session, target, format, sample rate, channel plan, capture, ASR, frontend, or application boundary.
