...
`just ci` is the deterministic project gate. It includes Rust checks, D-Bus integration, retained-addon checks, staged integration, temporary-HOME user-install smokes, and lightweight Arch plus RPM metadata validation. Live desktop, microphone, and full package builds are excluded by design.
