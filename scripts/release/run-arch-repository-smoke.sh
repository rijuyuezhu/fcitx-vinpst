...
assert_repository_version() {
  ...
  grep -qx 'Name            : fcitx-vinput-rs' <<<"${info}"
  grep -qx "Version         : ${expected_version}" <<<"${info}"
  grep -qx 'Architecture    : x86_64' <<<"${info}"
  grep -qx 'Provides        : fcitx5-vinput=0.1.0' <<<"${info}"
  grep -qx 'Conflicts With  : fcitx5-vinput' <<<"${info}"
}
...
grep -qx "fcitx-vinput-rs-${upgrade_version}/" <<<"${repository_entries}"
! grep -q "fcitx-vinput-rs-${initial_version}/" <<<"${repository_entries}"
