...
def assemble(args: argparse.Namespace) -> None:
    ...
    for role, raw_source in args.artifact:
        ...
        if source == bundle or bundle in source.parents:
            raise ValueError(
                f"artifact must not be inside the output directory: {source}"
            )
    ...
    staging = Path(tempfile.mkdtemp(prefix=f".{bundle.name}.tmp-", dir=bundle.parent))
    try:
...
def verify_bundle(bundle: Path) -> None:
    if bundle.is_symlink() or not bundle.is_dir():
        raise ValueError(
            f"release bundle directory is missing or is a symlink: {bundle}"
        )
    ...
    checksum_size = require_nonnegative_integer(
        checksum_file.get("size"), "checksum_file.size"
    )
    checksum_digest = require_sha256(
        checksum_file.get("sha256"), "checksum_file.sha256"
    )
    ...
    for artifact in artifacts:
        ...
        if not artifact_path.is_file() or artifact_path.is_symlink():
            raise ValueError(
                f"artifact is missing or not a regular file: {artifact_path.name}"
            )
    ...
    if actual_names != expected_names:
        ...
        raise ValueError(
            f"release bundle inventory mismatch: extras={extras}, missing={missing}"
        )
