...
redirect_marker="redirect-denied-marker"
...
write_config "${redirect_origin_url}" 2000
...
write_config "${oversized_success_url}" 2000
run_daemon_failure oversized-success "${clear_proxy_env[@]}"
wait_fixture "${oversized_success_pid}"
grep -Fq 'remote ASR HTTP response body exceeds 1048576-byte limit'   "${out_dir}/oversized-success.stderr"
...
oversized_error_marker="oversized-asr-error-marker"
...
write_config 'http://remote-asr-dns-failure.invalid/v1?api-key=diagnostic-url-secret#fragment' 5000
