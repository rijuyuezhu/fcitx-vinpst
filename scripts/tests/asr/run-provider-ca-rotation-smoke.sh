...
out_dir="${VINPUT_PROVIDER_CA_ROTATION_DIR:-target/tmp/provider-ca-rotation-smoke}"
...
export VINPUT_ROTATION_OUT_DIR="${out_dir}"
export VINPUT_ROTATION_ASR_FIXTURE="${asr_fixture}"
export VINPUT_ROTATION_TEXT_FIXTURE="${text_fixture}"
export VINPUT_ROTATION_DAEMON="${daemon}"
export VINPUT_ROTATION_CLI="${cli}"
export VINPUT_ROTATION_WAV="${wav_file}"
export VINPUT_ROTATION_ASR_CONFIG="${asr_config}"
export VINPUT_ROTATION_TEXT_CONFIG="${text_config}"
export VINPUT_ROTATION_TRUSTED_CA="${trusted_ca}"
export VINPUT_ROTATION_CA_A="${ca_a_cert}"
export VINPUT_ROTATION_CA_B="${ca_b_cert}"
export VINPUT_ROTATION_SERVER_A_CERT="${server_a_cert}"
export VINPUT_ROTATION_SERVER_A_KEY="${server_a_key}"
export VINPUT_ROTATION_SERVER_B_CERT="${server_b_cert}"
export VINPUT_ROTATION_SERVER_B_KEY="${server_b_key}"
export VINPUT_ROTATION_ASR_PORT="${asr_port}"
export VINPUT_ROTATION_TEXT_PORT="${text_port}"
export VINPUT_ROTATION_API_KEY="${api_key}"
export VINPUT_ROTATION_MODEL="${model}"
export VINPUT_ROTATION_LANGUAGE="${language}"
export VINPUT_ROTATION_PROMPT="${prompt}"
export VINPUT_ROTATION_SELECTED="${selected_text}"
...
out_dir="${VINPUT_ROTATION_OUT_DIR}"
origin_pid=""
daemon_pid=""
...
start_origin() {
  ...
  python3 "${VINPUT_ROTATION_ASR_FIXTURE}" \
    --ready-file "${ready}" \
    --trace-file "${trace}" \
    --error-file "${error}" \
    --port "${VINPUT_ROTATION_ASR_PORT}" \
    --api-key "${VINPUT_ROTATION_API_KEY}" \
    --model "${VINPUT_ROTATION_MODEL}" \
    --language "${VINPUT_ROTATION_LANGUAGE}" \
    --prompt "${VINPUT_ROTATION_PROMPT}" \
  ...
  jq -e --argjson port "${VINPUT_ROTATION_ASR_PORT}" \
...
assert_idle_owner() {
  ...
  local output="$2"
  "${VINPUT_ROTATION_CLI}" daemon status --json >"${output}"
...
recognize_success() {
  local name="$1"
  local expected="$2"
  "${VINPUT_ROTATION_CLI}" recording start --json \
    >"${out_dir}/${name}.start.json"
  "${VINPUT_ROTATION_CLI}" recording stop --json \
...
start_origin asr-ca-a "${VINPUT_ROTATION_SERVER_A_CERT}" \
  "${VINPUT_ROTATION_SERVER_A_KEY}" "rotation asr a"
env \
  -u ALL_PROXY -u all_proxy \
  -u HTTP_PROXY -u http_proxy \
  -u HTTPS_PROXY -u https_proxy \
  -u NO_PROXY -u no_proxy \
  SSL_CERT_FILE="${VINPUT_ROTATION_TRUSTED_CA}" \
  "${VINPUT_ROTATION_DAEMON}" \
    --dbus \
    --configured-backends \
    --config "${VINPUT_ROTATION_ASR_CONFIG}" \
    --wav "${VINPUT_ROTATION_WAV}" \
...
for _ in $(seq 1 100); do
  if busctl --user --no-pager --list | awk -v pid="${daemon_pid}" '
    $1 == "org.fcitx.Vinput" && $2 == pid { found = 1 }
    END { exit !found }
  '; then
    "${VINPUT_ROTATION_CLI}" daemon status --json \
...
cp "${VINPUT_ROTATION_CA_B}" "${VINPUT_ROTATION_TRUSTED_CA}.next"
chmod 600 "${VINPUT_ROTATION_TRUSTED_CA}.next"
mv "${VINPUT_ROTATION_TRUSTED_CA}.next" "${VINPUT_ROTATION_TRUSTED_CA}"
start_origin asr-ca-mismatch "${VINPUT_ROTATION_SERVER_A_CERT}" \
  "${VINPUT_ROTATION_SERVER_A_KEY}" "unused"
"${VINPUT_ROTATION_CLI}" recording start --json \
...
start_origin asr-ca-b "${VINPUT_ROTATION_SERVER_B_CERT}" \
  "${VINPUT_ROTATION_SERVER_B_KEY}" "rotation asr b"
...
start_origin() {
  ...
  local log="${out_dir}/${name}.fixture.log"
  rm -f "${ready}" "${trace}" "${error}" "${log}"
  python3 "${VINPUT_ROTATION_TEXT_FIXTURE}" \
    --ready-file "${ready}" \
    --trace-file "${trace}" \
    --error-file "${error}" \
    --port "${VINPUT_ROTATION_TEXT_PORT}" \
    --api-key "${VINPUT_ROTATION_API_KEY}" \
    --model "${VINPUT_ROTATION_MODEL}" \
  ...
  origin_pid=$!
  wait_ready "${origin_pid}" "${ready}" "${log}"
  jq -e --argjson port "${VINPUT_ROTATION_TEXT_PORT}" \
    '.tls == true and .port == $port' "${ready}" >/dev/null
}
...
command_success() {
  local name="$1"
  local prefix="$2"
  local expected="${prefix}${VINPUT_ROTATION_SELECTED} | command: mock recognition result"
  "${VINPUT_ROTATION_CLI}" recording start \
    --selected-text "${VINPUT_ROTATION_SELECTED}" \
    --json >"${out_dir}/${name}.start.json"
  "${VINPUT_ROTATION_CLI}" recording stop \
    --scene __command__ \
...
start_origin text-ca-a "${VINPUT_ROTATION_SERVER_A_CERT}" \
  "${VINPUT_ROTATION_SERVER_A_KEY}" "rotation text a: "
env \
  -u ALL_PROXY -u all_proxy \
  -u HTTP_PROXY -u http_proxy \
  -u HTTPS_PROXY -u https_proxy \
  ...
  "${VINPUT_ROTATION_DAEMON}" \
    --dbus \
    --configured-backends \
    --config "${VINPUT_ROTATION_TEXT_CONFIG}" \
    --wav "${VINPUT_ROTATION_WAV}" \
...
chmod 600 "${VINPUT_ROTATION_TRUSTED_CA}.next"
mv "${VINPUT_ROTATION_TRUSTED_CA}.next" "${VINPUT_ROTATION_TRUSTED_CA}"
start_origin text-ca-mismatch "${VINPUT_ROTATION_SERVER_A_CERT}" \
  "${VINPUT_ROTATION_SERVER_A_KEY}" "unused: "
"${VINPUT_ROTATION_CLI}" recording start \
  --selected-text "${VINPUT_ROTATION_SELECTED}" \
...
start_origin text-ca-b "${VINPUT_ROTATION_SERVER_B_CERT}" \
  "${VINPUT_ROTATION_SERVER_B_KEY}" "rotation text b: "
