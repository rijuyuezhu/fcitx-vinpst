...
config_path="${data_home}/fcitx-vinpst/e2e-command-demo-config.json"
...
common_env=(
  ...
  VINPST_USER_BIN_DIR="${bin_dir}"
  VINPST_USER_FCITX_LIB_DIR="${lib_dir}"
  VINPST_USER_PROFILE=command-demo
  VINPST_USER_RUNTIME_ACTIVATION=1
)

env "${common_env[@]}" scripts/install/install-user-ime.sh >"${out_dir}/install.log" 2>&1
...
grep -Fq -- "--wav ${data_home}/fcitx-vinpst/e2e-command-demo.wav" "${service_file}"

HOME="${home_dir}" \
XDG_DATA_HOME="${data_home}" \
XDG_DATA_DIRS="${data_home}:${XDG_DATA_DIRS:-/usr/local/share:/usr/share}" \
XDG_RUNTIME_DIR="${runtime_dir}" \
VINPST_OWNER_SMOKE_STATUS="${status_output}" \
VINPST_OWNER_SMOKE_DAEMON="${daemon_path}" \
  timeout 20s dbus-run-session -- bash -euo pipefail <<'INNER'
stop_activation_owner() {
  if [[ ! -f "${VINPST_OWNER_SMOKE_STATUS}" ]]; then
    return 0
  fi
  local owner_pid owner_exe owner_cmd0 expected_exe
  owner_pid="$(jq -r '.owner.unix_process_id // empty' "${VINPST_OWNER_SMOKE_STATUS}")"
  if [[ ! "${owner_pid}" =~ ^[0-9]+$ ]]; then
    return 0
  fi
  owner_exe="$(readlink "/proc/${owner_pid}/exe" 2>/dev/null || true)"
  owner_cmd0="$(tr '\0' '\n' <"/proc/${owner_pid}/cmdline" 2>/dev/null | head -n 1)"
  expected_exe="$(realpath "${VINPST_OWNER_SMOKE_DAEMON}")"
  if [[ "${owner_cmd0}" != "${VINPST_OWNER_SMOKE_DAEMON}" ]] ||
...
target/debug/vinpst daemon status --json >"${VINPST_OWNER_SMOKE_STATUS}"
python3 - "${VINPST_OWNER_SMOKE_STATUS}" "${VINPST_OWNER_SMOKE_DAEMON}" <<'PY'
import json
import pathlib
import sys
...
env "${common_env[@]}" VINPST_USER_REMOVE=1 scripts/install/install-user-ime.sh >/dev/null
