#!/usr/bin/env python3
"""Act as one deterministic HTTP proxy for an OpenAI-compatible ASR request."""
...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if (
            ...
            or target.path != "/v1/audio/transcriptions"
        ):
            self.send_json(502, {"error": "unexpected proxy target"})
        ...
        if proxy_authorization != expected_proxy_authorization:
            self.send_json(407, {"error": "missing or invalid proxy authorization"})
            threading.Thread(target=self.server.shutdown, daemon=True).start()
            return
        if authorization != f"Bearer {args.api_key}":
            self.send_json(407, {"error": "missing or invalid bearer token"})
            threading.Thread(target=self.server.shutdown, daemon=True).start()
            return
        if not content_type.lower().startswith("multipart/form-data;"):
            self.send_json(400, {"error": "unexpected content type"})
            threading.Thread(target=self.server.shutdown, daemon=True).start()
            return
        if content_length <= 0:
        ...
        if b"RIFF" not in body or args.model.encode() not in body:
            self.send_json(400, {"error": "invalid multipart payload"})
            threading.Thread(target=self.server.shutdown, daemon=True).start()
            return
...
def parse_args() -> argparse.Namespace:
    ...
    for name in ("api_key", "expected_host", "model", "response_text"):
