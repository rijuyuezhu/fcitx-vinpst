...
def load_request() -> dict[str, Any]:
    ...
    if not isinstance(request, dict):
        raise ValueError("command ASR request must be a JSON object")
    return request
...
def pcm_samples(request: dict[str, Any]) -> list[int]:
    ...
    if not isinstance(samples, list):
        raise ValueError("samples must be an array of signed 16-bit integers")
    result: list[int] = []
...
def command_env(request: dict[str, Any], wav_path: Path, sample_rate: int, channels: int) -> dict[str, str]:
    env = os.environ.copy()
    env.update(
        {
...
def main() -> int:
    ...
    try:
        ...
        timeout_ms = args.timeout_ms if args.timeout_ms is not None else request.get("timeout_ms")
        timeout_s = None if timeout_ms in (None, "") else max(int(timeout_ms), 1) / 1000.0
        ...
        if completed.returncode != 0:
            message = stderr or stdout or f"external ASR command exited with {completed.returncode}"
            return response({"error": message})
        if not stdout:
            return response({"error": "external ASR command produced no text"})
        return response({"text": stdout})
    except subprocess.TimeoutExpired as error:
        return response({"error": f"external ASR command timed out after {error.timeout:.3g}s"})
    except Exception as error:  # Keep helper errors inside the command-ASR protocol.
