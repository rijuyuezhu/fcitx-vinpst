...
binary="${VINPST_WHISPER_CPP_BINARY:-${build_dir}/bin/whisper-cli}"
default_model="${HOME}/.local/share/voxtype/models/ggml-base.bin"
model="${VINPST_WHISPER_MODEL:-${default_model}}"
wav="${VINPST_WHISPER_WAV:-target/models/onnx-zf-ctc-zh-sm-int8-stream/test_wavs/0.wav}"
language="${VINPST_WHISPER_LANGUAGE:-zh}"
threads="${VINPST_WHISPER_THREADS:-8}"
timeout_seconds="${VINPST_WHISPER_TIMEOUT_SECONDS:-120}"
out_dir="${VINPST_WHISPER_OUT_DIR:-target/tmp/whisper-cpp-asr-live}"
expected_model_sha256="${VINPST_WHISPER_EXPECTED_MODEL_SHA256:-}"
