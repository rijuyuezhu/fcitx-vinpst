...
- `crates/vinput-process`: shared Unix helper supervision, whole-process-group cleanup, deadlines, and bounded output capture.
- `crates/vinput-audio`: PCM types, audio transforms, recorder traits, and optional PipeWire capture.
- `crates/vinput-asr`: ASR traits plus mock, supervised command, remote, and optional native `sherpa-onnx` backends.
...
- F10 -> selected surrounding text -> live partials -> deletion -> an `adapter-backed:` direct replacement commit from the configured local command adapter; command text and command ASR helpers share isolated process-group supervision, terminate descendants on configured deadlines or output failure, clean up descendants after direct-child exit, and cap stdout and stderr independently at 1 MiB;
...
- real hosted ASR and cloud text-provider operations, including third-party DNS/TLS, PAC, NTLM/Kerberos, enterprise TLS-interception policy and certificate deployment, provider-specific rate-limit/outage policy, provider credential rotation/custody, and production CA distribution/revocation; both production clients have deterministic local plain-HTTP proxy routing, proxy-URL Basic authentication for direct HTTP over plain-HTTP proxies and HTTPS CONNECT through both plain-HTTP and TLS-protected HTTPS proxy endpoints, `NO_PROXY`, additional PEM roots from `SSL_CERT_FILE` while retaining built-in `WebPKI` roots and certificate verification, a local CA-signed TLS interception relay that retains only TLS versions and byte counts, same-daemon atomic replacement of one `SSL_CERT_FILE` path with mismatch rejection and idle recovery, 429/503, fail-closed 3xx handling that never contacts `Location`, separately classified request and response-body timeouts, a 1 MiB cap for success and error response bodies, untrusted self-signed TLS rejection, DNS failure, connection-refusal, and credential-redaction coverage. The text-provider path additionally applies the legacy 4000 ms scene timeout whenever `timeout_ms` is omitted. Provider diagnostics remove URL userinfo/fragments, retain only query keys with `REDACTED` values, hide ASR prompts and text request bodies from `Debug`, and sanitize known credentials echoed by HTTP error bodies without changing the actual request URL or payload;
