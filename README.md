...
- an Arch Linux `x86_64` package recipe covering the Rust binaries, Fcitx addon, systemd/D-Bus activation, translations, VAD asset, private native runtime bundle, message-only lifecycle guidance, local pacman repository install/upgrade, signed-repository trust/tamper validation with ephemeral keys, and a machine-verified release-gate artifact manifest plus `SHA256SUMS`, and detached manifest signing verified against an external pinned key;
