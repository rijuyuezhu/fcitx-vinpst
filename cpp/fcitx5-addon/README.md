...
Status/menu calls use the safe Rust blocking transport with the frozen five-second D-Bus deadline. Recording Start/Stop instead use Fcitx `callAsync` with the same five-second deadline so ASR and post-processing never block the Fcitx event loop; Rust still prepares the exact operation/argument and consumes the final `RecognitionResult` through the narrow FFI boundary. Transport failures activate the frozen 1.5-second retry cooldown. `scripts/tests/check_fcitx_ffi_abi.py` builds the static library and requires its complete `vinpst_fcitx_*` symbol set to match the public C header exactly; the normal C++ smoke suite then exercises the opaque ownership and view contracts through that header.
