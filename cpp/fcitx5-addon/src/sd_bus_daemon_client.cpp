...
using OwnedStringHandle =
    RustOwnedHandle<VinpstFcitxOwnedString, vinpst_fcitx_owned_string_free>;
using DaemonClientHandle =
    RustOwnedHandle<VinpstFcitxDaemonClient, vinpst_fcitx_daemon_client_free>;

std::string CopyOwnedString(const VinpstFcitxOwnedString *value) {
  VinpstFcitxStringView view{};
  if (value == nullptr || vinpst_fcitx_owned_string_view(value, &view) == 0) {
    return {};
  }
...
void SetRustError(std::string *error, VinpstFcitxOwnedString *raw_error,
...
struct SdBusDaemonClient::Impl {
  explicit Impl(VinpstFcitxDaemonClient *client)
...
SdBusDaemonClient::ConnectSession(std::string *error) {
  VinpstFcitxOwnedString *raw_error = nullptr;
  auto *client = vinpst_fcitx_daemon_client_connect(&raw_error);
  if (client == nullptr) {
...
SdBusDaemonClient::SdBusDaemonClient(VinpstFcitxDaemonClient *client)
...
bool SdBusDaemonClient::GetStatus(std::string *status, std::string *error) {
  ...
  auto status_text = OwnedStringHandle::Adopt(
      vinpst_fcitx_daemon_client_get_status(impl_->client.raw_handle(), &raw_error));
  if (!status_text) {
...
bool SdBusDaemonClient::RefreshSceneMenuController(SceneMenuController *controller,
  ...
  VinpstFcitxOwnedString *raw_error = nullptr;
  if (vinpst_fcitx_daemon_client_refresh_scene_menu_controller(
...
bool SdBusDaemonClient::SetActiveScene(SceneMenuController *controller,
  ...
  VinpstFcitxOwnedString *raw_error = nullptr;
  if (vinpst_fcitx_daemon_client_set_active_scene(
...
bool SdBusDaemonClient::RefreshAsrMenuController(AsrMenuController *controller,
  ...
  VinpstFcitxOwnedString *raw_error = nullptr;
  if (vinpst_fcitx_daemon_client_refresh_asr_menu_controller(
          impl_->client.raw_handle(), controller->mutable_raw_handle(), &raw_error) ==
      0) {
    SetRustError(error, raw_error,
...
bool SdBusDaemonClient::SetActiveAsrTarget(std::string_view provider_id,
  ...
  const VinpstFcitxAsrTargetView target{
      .provider = ToRustStringView(provider_id),
      .model = ToRustStringView(model_value),
  };
  std::uint8_t persisted_value = 0;
  VinpstFcitxOwnedString *raw_error = nullptr;
  if (vinpst_fcitx_daemon_client_set_active_asr_target(
...
const VinpstFcitxDaemonClient *SdBusDaemonClient::raw_handle() const {
