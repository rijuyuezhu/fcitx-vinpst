...
# kitty path with terminal output bytes, primary-selection fallback, niri focus, and isolated audio.
ime-kitty-virtual-source-live mode='normal':
    VINPUT_LIVE_VIRTUAL_PROBE_KIND=kitty VINPUT_LIVE_TOOLKIT_MODE="{{mode}}" VINPUT_LIVE_VIRTUAL_OUT_DIR="target/tmp/ime-kitty-virtual-source-live/{{mode}}" scripts/run-ime-fcitx-virtual-source-live.sh
...
# Run legacy raw-PCM command ASR WAV bridge smoke.
